...
const resolvedBuildProfile = String(process.env.EXPO_PUBLIC_BUILD_PROFILE || '').trim().toLowerCase()

if (resolvedBuildProfile !== String(buildProfile || '').trim().toLowerCase()) {
  console.error(`Resolved ${appRole || 'unknown'} ${buildProfile || 'unknown'} environment failed closed.`)
  console.error('build_profile_mismatch')
  process.exit(1)
}
