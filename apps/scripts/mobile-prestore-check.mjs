...
assertIncludes(coachMatchDayCore, "number < 0 || number > 130", 'Coach Match Day minute validation')
...
assertIncludes(coachMatchDayData, "rpc('record_match_day_goal_v2'", 'Coach Match Day goal authority')
assertIncludes(coachMatchDayData, "rpc('record_match_day_score_correction_v2'", 'Coach Match Day score correction authority')
assertIncludes(coachMatchDayData, "rpc('record_match_day_staff_event_v2'", 'Coach Match Day event authority')
...
assertIncludes(coachMatchDayScreen, 'Showing encrypted cached Match Day data.', 'Coach Match Day offline read state')
assertIncludes(coachMatchDayScreen, 'Every change is disabled until a successful refresh.', 'Coach Match Day online-only writes')
...
if (existsSync(join(repoRoot, sharedPrivacyPath))) {
  ...
  assertIncludes(privacyQuestionnaire, 'Do not declare tracking unless a future analytics or advertising SDK is deliberately added and this questionnaire is revised.', 'Mobile privacy questionnaire')
