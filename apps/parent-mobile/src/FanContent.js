...
export function FanContent({ connection, view, content, formation, onCloseFormation, onOpenResource, onOpenLink, onOpen, themeTokens }) {
  ...
  if (view.action === 'matches') {
    const isPlayer = connection?.relationship_type === 'player'
    const matches = (content.matches || []).filter((item) => isFanGameDayMatch(item) || (isPlayer && item.status === 'scheduled')).map((item) => ({ ...normalizeParentMatchDay({ ...item, team_name: connection?.team_name, club_name: item.club_name || connection?.club_name, events: view.matchId === item.id ? content.events || [] : [], is_scorer: false, isScorer: false, request_scorer: false }), isFanView: true, canViewSelectedSquad: isPlayer }))
    if (!view.matchId || !matches.some(item => item.id === view.matchId)) return <View style={{ gap: 16 }}>
      ...
      <Text style={{ color: themeTokens.textSecondary }}>{isPlayer ? 'Upcoming fixtures, live matches and results shared for' : 'Live matches and results shared for'} {connection?.player_name || 'your player'}.</Text>
      {(isPlayer ? [{ label: 'Upcoming', items: matches.filter(item => item.status === 'scheduled').sort((a, b) => Date.parse(a.matchDate) - Date.parse(b.matchDate)) }, { label: 'Live matches', items: matches.filter(item => item.status !== 'full_time' && item.status !== 'scheduled') }, { label: 'Results', items: matches.filter(item => item.status === 'full_time') }] : [{ label: 'Live matches', items: matches.filter(item => item.status !== 'full_time') }, { label: 'Results', items: matches.filter(item => item.status === 'full_time') }]).map(group => <View key={group.label} style={{ gap: 12 }}>
