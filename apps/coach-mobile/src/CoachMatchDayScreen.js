  ...
  return <View style={styles.stack}>
    ...
    {actionSheet?.kind === 'event' ? <MatchDayActionSheet busy={busy} capturedClock={actionSheet.capturedClock} onClose={() => setActionSheet(null)} styles={styles} title={actionSheet.title}>
      ...
      {eventForm.eventType === 'goal' ? <>
        {eventForm.teamSide === 'club' ? <><Text style={styles.fieldLabel}>Scorer type</Text><Chips onChange={(value) => onEventForm({ ...eventForm, scorerName: '', scorerParticipantType: value, scorerShirtNumber: '' })} options={participantOptions} styles={styles} value={eventForm.scorerParticipantType} /></> : null}
        {eventForm.teamSide === 'opponent' || eventForm.scorerParticipantType === 'player'
