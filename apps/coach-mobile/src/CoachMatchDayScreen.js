...
function createStyles(palette) {
  return StyleSheet.create({
    action: { alignItems: 'center', backgroundColor: palette.accent, borderColor: palette.accentText, borderWidth: 1, borderRadius: 13, flexDirection: 'row', gap: 7, justifyContent: 'center', minHeight: 48, paddingHorizontal: 14, paddingVertical: 10 },
    actionDanger: { backgroundColor: palette.danger },
    actionDangerText: { color: palette.dangerForeground },
    ...
    playerChoiceToggleText: { color: palette.accentText, fontSize: 12, fontWeight: '900' },
    ...
    capturedPill: { alignSelf: 'flex-start', backgroundColor: palette.selected, borderColor: palette.accentText, borderRadius: 999, borderWidth: 1, color: palette.selectedForeground, fontSize: 12, fontWeight: '900', overflow: 'hidden', paddingHorizontal: 10, paddingVertical: 6 },
...
function TimelinePanel({ busy, match, onCorrectGoal, onPrepare, onUndo, styles }) {
  ...
  return <View style={styles.stack}>
    ...
    {visibleEvents.map((event) => { const undo = getCoachMatchDayUndoModel(event); const presentation = buildCompletedMatchEventPresentation(event, match, { includeNotes: false }); const isCorrecting = correctEvent?.id === event.id; return <View key={event.id} style={styles.card}><Text style={styles.cardTitle}>{presentation.title} {presentation.minuteLabel}</Text><Text style={styles.body}>{presentation.detail || event.notes || event.teamSide}</Text><Text style={styles.meta}>{event.homeScore} - {event.awayScore} | {event.eventStatus}</Text>{event.eventType === 'goal' && event.eventStatus !== 'voided' ? <Button disabled={busy} label={isCorrecting ? 'Close goal correction' : 'Correct goal details'} onPress={() => { if (isCorrecting) { setCorrectEvent(null); setGoalDraft(null); return } setCorrectEvent(event); setCorrectionReason(''); setGoalDraft({ isOwnGoal: event.isOwnGoal === true, stoppageMinute: String(event.stoppageMinute ?? ''), assistName: event.assistName, assistShirtNumber: event.assistShirtNumber, minute: String(event.minute ?? ''), notes: event.notes, scorerName: event.scorerName, scorerShirtNumber: event.scorerShirtNumber, teamSide: event.teamSide }) }} secondary styles={styles} /> : null}{isCorrecting ? renderGoalCorrection() : null}{undo.canUndo ? <Button disabled={busy} label="Undo event" onPress={() => { setUndoEvent(event); setReasonCode(''); setNote('') }} secondary styles={styles} /> : null}</View> })}
...
const AI_REPORT_PILOT_CLUB_ID = 'f747d2cc-a5e5-4960-8ad3-dcfe85b49279'
const aiQuestions = [
...
function CoachAiReport({ match, onSaved, user, styles }) {
  ...
  const sharePdf = async () => {
    setWorking('pdf'); setFeedback('')
    try {
      if (!await Sharing.isAvailableAsync()) throw new Error('PDF sharing is unavailable on this device.')
      const bytes = buildCompletedReportPdf(match, { audience: 'staff', coachNarrative: savedText, accessContext: { planKey: 'matchday' } })
      const file = new File(Paths.cache, `coach-match-report-${match.id}.pdf`)
      file.create({ overwrite: true })
      file.write(bytes)
      await Sharing.shareAsync(file.uri, { dialogTitle: 'Save or share coach match report', mimeType: 'application/pdf', UTI: 'com.adobe.pdf' })
    } catch (error) { setFeedback(error.message || 'The PDF could not be prepared.') }
    finally { setWorking('') }
  }
  return <View style={styles.card}>
...
export function CoachMatchDayScreen({ context, matchDayTarget, onMatchDayTargetHandled, onNavigate, onQuickActionHandled, onRequestScrollTop, palette, quickAction, user }) {
  const templateStore = useMemo(() => createCoachSquadTemplateStore(user), [user])
  ...
  const reviewSavedActions = async () => {
    setBusy(true)
    try {
      const latest = await getCoachMatchDayDetail(user, match.id)
      setMatch(latest)
      setPending({ kind: 'review-saved', label: `Apply the ${pendingCount} saved ${pendingCount === 1 ? 'action' : 'actions'} shown above to the latest match at ${getCoachMatchDayPresentation(latest).displayScore}? Review the current score and events first.`,
        run: async () => { try { await outbox.review(latest); await outbox.retry() } catch (reviewError) { setError(errorMessage(reviewError, 'Saved actions could not be reviewed.')) } } })
    } catch (reviewError) { setError(errorMessage(reviewError, 'The latest match could not be loaded.')) }
    finally { setBusy(false) }
  }
  const reportMatch = pendingCount > 0 && serverMatch?.status === 'full_time' ? serverMatch : match
