...
import { ActivityIndicator, AppState, Dimensions, Image, KeyboardAvoidingView, Linking, Modal, Platform, Pressable, ScrollView, Text, TextInput, View } from 'react-native'
...
export function PartnersScreen({ textStyle, headingStyle, appRole }) {
  ...
  return <View style={{gap:16,paddingVertical:8}} onLayout={(e)=>setWidth(e.nativeEvent.layout.width)}>
    ...
    <Modal visible={Boolean(reporting)} transparent animationType="fade" onRequestClose={()=>setReporting(null)}><KeyboardAvoidingView behavior={Platform.OS==='ios'?'padding':'height'} style={{flex:1,backgroundColor:'#0008'}}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{flexGrow:1,justifyContent:'center',padding:24}}><View style={{backgroundColor:'white',borderRadius:16,padding:20,gap:14}}><Text style={{fontSize:20,fontWeight:'800',color:'#182e23'}}>Report {reporting?.title}</Text><Text style={{color:'#405b4c'}}>Tell us what is wrong with this offer. Avoid including private player information.</Text><TextInput accessibilityLabel="Reason for reporting" value={reason} onChangeText={setReason} multiline maxLength={1000} placeholder="Describe the issue" placeholderTextColor="#65796e" style={{minHeight:100,borderWidth:1,borderColor:'#b8ccc0',borderRadius:8,padding:12,color:'#182e23',textAlignVertical:'top'}}/><Pressable style={actionStyle} accessibilityRole="button" disabled={busy||reason.trim().length<3} onPress={()=>void sendReport()}><Text style={actionText}>{busy?'Sending...':'Send report'}</Text></Pressable><Pressable style={actionStyle} accessibilityRole="button" onPress={()=>setReporting(null)}><Text style={actionText}>Cancel</Text></Pressable>{!!notice&&<Text style={{color:'#9b1c1c'}}>{notice}</Text>}</View></ScrollView></KeyboardAvoidingView></Modal>
  </View>
}
