...
export const PARENT_PORTAL_ACTIVITY_REGISTRY = Object.freeze([
  Object.freeze({
    ...
    eligibility: 'Active Parent link for the selected player and current Parent-visible event scope',
    markViewedAfter: 'Calendar data loads successfully for the selected player',
  }),
  Object.freeze({
    key: 'invites',
    route: '/parent-portal?section=invites',
    scope: PARENT_PORTAL_ACTIVITY_SCOPES.child,
    source: 'Active calendar and Match Day invitations for the selected player',
    eligibility: 'Active Parent link and non-cancelled invitation for the selected player',
    markViewedAfter: 'Invitation data loads successfully for the selected player',
  }),
  Object.freeze({
    key: 'matches',
    ...
    markViewedAfter: 'Current match-card data loads successfully for the selected player',
  }),
  Object.freeze({
    key: 'results',
    ...
    markViewedAfter: 'Results data loads successfully for the selected player',
  }),
  Object.freeze({
    key: 'resources',
    route: '/parent-portal?section=resources',
    scope: PARENT_PORTAL_ACTIVITY_SCOPES.child,
    source: 'Active Team Resource assignments shared with the selected player',
    eligibility: 'Active Parent link, active Player, active assignment and Parent-visible resource',
    markViewedAfter: 'Resource data loads successfully for the selected player',
  }),
  Object.freeze({
    key: 'chat',
    route: '/parent-chat',
    ...
    eligibility: 'Active Parent link and direct, team or selected match Chat relevance for the selected player',
    markViewedAfter: 'An authorised Chat room relevant to the selected player loads successfully',
  }),
  Object.freeze({
    key: 'polls',
    route: '/parent-polls',
    scope: PARENT_PORTAL_ACTIVITY_SCOPES.child,
    source: 'Open Parent polls available to the selected player link',
    eligibility: 'Active Parent link and current open Parent poll scope',
    markViewedAfter: 'Poll data loads successfully for the selected player',
  }),
])
