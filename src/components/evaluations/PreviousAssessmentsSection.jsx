        ...
        {previousAssessmentItems.length > 0 ? (
          previousAssessmentItems.map((item) => (
            <div key={item.label} className="rounded-2xl border border-slate-200 bg-white px-3 py-2">
              ...
              <p className="mt-1 whitespace-pre-wrap text-sm text-slate-600">{item.value}</p>
            </div>
          ))
        ) : (
          <div className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-600 md:col-span-2">
