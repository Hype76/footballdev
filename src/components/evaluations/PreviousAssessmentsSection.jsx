    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          type="button"
          onClick={onToggle}
          className="inline-flex min-h-11 items-center justify-center rounded-lg border border-[#bddcca] bg-white px-4 py-3 text-sm font-bold text-[#10231a] transition hover:bg-[#f6fbf8]"
        >
...
function PreviousAssessmentCard({ evaluation }) {
  ...
  return (
    <div className="rounded-lg border border-[#bddcca] bg-[#f6fbf8] p-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <p className="font-black text-[#10231a]">{evaluation.date || 'No date entered'}</p>
        <p className="text-sm font-black text-[#067a46]">
          Score: {evaluation.averageScore !== null ? evaluation.averageScore.toFixed(1) : '-'}
        </p>
      </div>
      <div className="mt-2 grid gap-1 text-sm font-semibold text-[#456653] sm:grid-cols-2">
      ...
      <div className="mt-3 grid gap-2 md:grid-cols-2">
        {previousAssessmentItems.length > 0 ? (
          previousAssessmentItems.map((item) => (
            <div key={item.label} className="rounded-lg border border-[#bddcca] bg-white px-3 py-2">
              <p className="text-xs font-black uppercase tracking-[0.12em] text-[#067a46]">{item.label}</p>
              <p className="mt-1 whitespace-pre-wrap text-sm font-semibold text-[#456653]">{item.value}</p>
            </div>
          ))
        ) : (
          <div className="rounded-lg border border-[#bddcca] bg-white px-3 py-2 text-sm font-semibold text-[#456653] md:col-span-2">
