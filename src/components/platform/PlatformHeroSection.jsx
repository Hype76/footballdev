        ...
        <div className="border border-emerald-200 bg-emerald-50 p-5">
          ...
          {actionLabel && onAction ? (
            <button
              type="button"
              onClick={onAction}
              className="mt-4 inline-flex min-h-11 w-full items-center justify-center border border-emerald-700 bg-emerald-700 px-4 py-3 text-sm font-bold text-white transition hover:bg-emerald-800"
            >
              {actionLabel}
            </button>
          ) : null}
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {items.map((item) => (
        <div
          key={item.label}
          className="border border-slate-200 bg-white p-5 transition hover:border-emerald-300"
        >
          <div className="flex items-start justify-between gap-4">
            ...
            {item.detail ? (
              <span className="mt-1 h-2.5 w-2.5 rounded-sm bg-emerald-600" />
            ) : null}
          </div>
