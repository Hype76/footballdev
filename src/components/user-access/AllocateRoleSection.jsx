...
export function AllocateRoleSection({
  ...
  return (
    <SectionCard
      title="Send Coach access"
      tourId="allocate-role-section"
      description={
        canAddMoreUsers
          ? 'Invite one Coach at a time. Pending means invited but not accepted, assigned means allocated by the Club Admin, and active means accepted and able to sign in.'
          : staffLimitMessage
      }
    >
      {isLoading ? (
        ...
        <form className={`${panelClass} p-5`} onSubmit={onSubmit}>
          <div className="mb-5">
            ...
            <p className={`mt-2 ${bodyTextClass}`}>
              Use the email they will sign in with. You can assign pending Coaches now, then they become active after accepting the invite.
            </p>
          </div>
