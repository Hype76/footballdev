...
export function SectionCard({
  ...
  return (
    <section
      data-tour-id={tourId}
      className="min-w-0 scroll-mt-28 overflow-hidden rounded-lg border border-[#cfeedd] bg-white shadow-sm shadow-[#d7eadf]/70"
    >
      <div className="flex flex-col gap-4 border-b border-[#cfeedd] bg-[#f8fdf9] px-5 py-5 sm:flex-row sm:items-start sm:justify-between sm:px-6">
        <div className="min-w-0">
          <h3 className="text-2xl font-black tracking-tight text-[#101828]">{title}</h3>
          {description ? <p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-[#5f7468]">{description}</p> : null}
        </div>
        <div className="flex w-full shrink-0 flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:justify-end">
          ...
          <button
            ...
            className="inline-flex min-h-10 min-w-[6.25rem] items-center justify-center whitespace-nowrap rounded-lg border border-[#cfeedd] bg-white px-4 py-2 text-sm font-black text-[#456653] transition hover:bg-[#f8fdf9]"
          >
