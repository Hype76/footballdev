...
export function UserFeedbackLinks({ onSelect }) {
  return <div className="flex flex-wrap gap-x-5 border-t border-[var(--border-color,#d7e5dc)] py-2">
    {[
      ...
      ['bug', 'Report a Bug'],
    ].map(([type, label]) => <Link key={type} data-tour-id={type === 'bug' ? 'sidebar-tester-feedback' : 'sidebar-user-feedback'} to={`/feedback/send?type=${type}`} onClick={onSelect} className="flex min-h-12 items-center gap-2 text-sm font-bold">
