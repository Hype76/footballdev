...
const panelClass = 'rounded-lg border border-[#cbd5e1] bg-white p-5 shadow-sm shadow-[#2563eb]/10 sm:p-6'
const eyebrowClass = 'text-xs font-black uppercase tracking-[0.18em] text-[#2563eb]'
const titleClass = 'mt-3 text-3xl font-black tracking-tight text-[#0f172a] sm:text-4xl'
const copyClass = 'mt-3 text-sm font-semibold leading-6 text-[#475569]'
const cardClass = 'rounded-lg border border-[#cbd5e1] bg-[#f8fafc] p-4 shadow-sm shadow-[#2563eb]/10'
...
const secondaryButtonClass = 'inline-flex min-h-12 items-center justify-center rounded-lg border border-[#cbd5e1] bg-white px-5 py-3 text-sm font-black text-[#0f172a] transition hover:bg-[#f8fafc]'
...
export function LoginMarketingAndPricing({
  ...
  return (
    <section className="space-y-5 pb-8 text-[#0f172a]">
      <div className={panelClass}>
        <p className={eyebrowClass}>Football-only club OS</p>
        <div className="mt-3 grid gap-5 lg:grid-cols-[1fr_0.8fr] lg:items-end">
          <div>
            <h2 className="max-w-4xl text-3xl font-black tracking-tight text-[#0f172a] sm:text-4xl">
              Built to move clubs away from scattered chats, paper forms, and generic team apps.
            </h2>
            <p className="mt-4 max-w-4xl text-base font-semibold leading-8 text-[#475569]">
          ...
          <div className="rounded-lg border border-[#bfdbfe] bg-[#eff6ff] p-4">
            <p className="text-sm font-black text-[#0f172a]">First useful action</p>
        ...
        <div className="grid gap-5 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {onboardingRules.map((rule) => (
              <div key={rule} className="rounded-lg border border-[#bfdbfe] bg-[#eff6ff] p-4 text-sm font-black leading-6 text-[#1d4ed8]">
                {rule}
              </div>
            ))}
          </div>
      ...
      <div className="flex flex-col gap-4 rounded-lg border border-[#cbd5e1] bg-white p-5 shadow-sm shadow-[#2563eb]/10 sm:p-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
        ...
        <div className="grid w-full max-w-xs grid-cols-2 rounded-lg border border-[#cbd5e1] bg-[#f8fafc] p-1">
          {[
            ['monthly', 'Monthly'],
            ['annual', 'Annual'],
          ].map(([key, label]) => (
            <button
              ...
              className={[
                ...
                billingCycle === key ? 'bg-[#2563eb] text-white shadow-sm shadow-[#2563eb]/20' : 'text-[#475569] hover:bg-white hover:text-[#0f172a]',
              ].join(' ')}
            >
              {label}
            </button>
      ...
      {localMessage ? (
        <div className="rounded-lg border border-[#bfdbfe] bg-[#eff6ff] px-4 py-3 text-sm font-black text-[#1d4ed8]">
          {localMessage}
        </div>
      ) : null}

      {paymentsDisabled ? (
        <div className="rounded-lg border border-[#bfdbfe] bg-[#eff6ff] px-5 py-4 text-sm font-black text-[#1d4ed8]">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {pricingPlans.map((plan) => {
          ...
          return (
            <div key={plan.name} className="relative flex flex-col rounded-lg border border-[#cbd5e1] bg-white p-5 shadow-sm shadow-[#2563eb]/10">
              {isPopular ? (
                <span className="absolute right-5 top-5 whitespace-nowrap rounded-lg border border-[#bfdbfe] bg-[#eff6ff] px-3 py-1 text-xs font-black text-[#1d4ed8]">
                  Popular
                </span>
              ) : null}
              {showPromotion ? (
                <div className="mb-4 rounded-lg border border-[#bfdbfe] bg-[#eff6ff] px-4 py-3 text-xs font-black uppercase tracking-[0.14em] text-[#1d4ed8]">
              ...
              <div className="min-h-[88px]">
                <span className={['whitespace-nowrap font-black text-[#0f172a]', plan.price === 'Contact us' ? 'text-[2rem] leading-none 2xl:text-4xl' : 'text-4xl'].join(' ')}>
                ...
                {typeof plan.price === 'number' && billingCycle === 'annual' ? (
                  <p className="mt-2 text-xs font-black text-[#2563eb]">2 months free compared with monthly</p>
                ) : null}
                {showPromotion ? (
      ...
      <footer className="flex flex-col gap-3 border-t border-[#cbd5e1] pt-6 text-sm font-semibold text-[#475569] sm:flex-row sm:items-center sm:justify-between">
        <p>Copyright 2026 Football Player. All rights reserved. Powered by <a href="https://pulseslabs.online" className="hover:text-[#0f172a]" target="_blank" rel="noreferrer">pulseslabs.online</a>.</p>
        <div className="flex flex-wrap gap-4">
          <a href="/gdpr" className="hover:text-[#0f172a]">GDPR</a>
          <a href="/terms" className="hover:text-[#0f172a]">Terms of Service</a>
        </div>
      </footer>
    </section>
  )
