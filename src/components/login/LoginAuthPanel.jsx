  ...
  return (
    <section ref={signupBoxRef}>
      <div className="mx-auto w-full max-w-md rounded-lg border border-[#cbd5e1] bg-white p-5 text-[#0f172a] shadow-lg shadow-[#0f172a]/10 sm:p-6">
        ...
        <div className="mt-5 grid grid-cols-2 rounded-lg border border-[#cbd5e1] bg-[#f8fafc] p-1">
          <button
            type="button"
            onClick={() => onModeChange('login')}
            className={[
              'min-h-11 rounded-lg px-4 py-3 text-sm font-black transition',
              mode === 'login' ? 'bg-[#2563eb] text-white shadow-sm shadow-[#2563eb]/20' : 'text-[#475569] hover:bg-white hover:text-[#0f172a]',
            ].join(' ')}
          >
            Login
          </button>
          <button
            ...
            className={[
              'min-h-11 rounded-lg px-4 py-3 text-sm font-black transition',
              mode === 'signup' ? 'bg-[#2563eb] text-white shadow-sm shadow-[#2563eb]/20' : 'text-[#475569] hover:bg-white hover:text-[#0f172a]',
            ].join(' ')}
          >
            Sign Up
          </button>
        ...
        <form className="mt-6 space-y-4" onSubmit={onSubmit}>
          {mode === 'signup' && !parentInviteMode ? (
            <label className="block">
              <span className="mb-2 block text-sm font-bold text-[#0f172a]">Club Name</span>
              <input
                type="text"
                name="clubName"
          ...
          {mode === 'signup' && !parentInviteMode ? (
            <label className="block">
              <span className="mb-2 block text-sm font-bold text-[#0f172a]">Tester access code</span>
              <input
                type="text"
                name="accessCode"
                ...
                className="min-h-12 w-full rounded-lg border border-[#cbd5e1] bg-[#f8fafc] px-4 py-3 text-sm font-semibold uppercase text-[#0f172a] outline-none transition placeholder:normal-case placeholder:text-[#94a3b8] focus:border-[#2563eb] focus:bg-white focus:ring-2 focus:ring-[#bfdbfe]"
              />
              <span className="mt-2 block text-xs leading-5 text-[#475569]">
          ...
          {localMessage ? (
            <div className="rounded-lg border border-[#bfdbfe] bg-[#eff6ff] px-4 py-3 text-sm font-bold text-[#1d4ed8]">
              {localMessage}
            </div>
          ) : null}
            ...
            {mode === 'login' ? (
              <>
                <button
                  ...
                  title={isSubmitting ? 'Please wait while your request is being checked.' : undefined}
                  onClick={onDemoLogin}
                  className="inline-flex min-h-12 w-full items-center justify-center rounded-lg border border-[#bfdbfe] bg-[#eff6ff] px-5 py-3 text-sm font-black text-[#1d4ed8] transition hover:bg-[#dbeafe] disabled:cursor-not-allowed disabled:opacity-60"
                >
                  Open Demo Account
                </button>
                <button
                  ...
                  className="inline-flex min-h-12 w-full items-center justify-center rounded-lg border border-[#cbd5e1] bg-white px-5 py-3 text-sm font-bold text-[#0f172a] transition hover:bg-[#f8fafc] disabled:cursor-not-allowed disabled:opacity-60"
                >
                  Forgot password
                </button>
              </>
