    ...
    <div className="space-y-5 sm:space-y-6">
      <section className="overflow-hidden rounded-lg border border-[#d7e5dc] bg-white shadow-sm shadow-[#047857]/10">
        <div className="grid gap-6 px-5 py-6 sm:px-6 lg:grid-cols-[minmax(0,1fr)_24rem] lg:items-stretch">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.18em] text-[#047857]">Communication queue</p>
          ...
          <div className="grid content-between rounded-lg border border-[#d7e5dc] bg-[#ecfdf5] p-5 shadow-sm shadow-[#047857]/10">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-[#047857]">Next send</p>
              ...
              <p className="mt-2 break-words text-sm font-semibold leading-6 text-[#4b5f55]">
      ...
      <div className="rounded-lg border border-[#d7e5dc] bg-white p-4 shadow-sm shadow-[#047857]/10 sm:p-5">
        <div className="mb-4 flex flex-col gap-2 border-b border-[#d7e5dc] pb-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.16em] text-[#047857]">Holding queue</p>
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {sortedQueue.map((item) => (
              <div key={item.id} className="rounded-lg border border-[#d7e5dc] bg-[#ecfdf5] p-4 shadow-sm shadow-[#047857]/10">
                <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-start">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="rounded-lg border border-[#bbf7d0] bg-white px-3 py-1 text-xs font-black uppercase tracking-[0.12em] text-[#047857]">
                        {item.status}
                      </span>
                      {item.hasAttachment ? (
                        <span className="rounded-lg border border-[#d7e5dc] bg-white px-3 py-1 text-xs font-black text-[#4b5f55]">
                          PDF attached
                        </span>
                      ) : null}
                    </div>
                    <h2 className="mt-3 break-words text-lg font-black text-[#101828]">{item.subject}</h2>
                    <p className="mt-2 break-words text-sm font-semibold leading-6 text-[#4b5f55]">To: {item.toEmail}</p>
                    <p className="mt-1 text-sm font-semibold leading-6 text-[#4b5f55]">Send time: {formatDateTime(item.scheduledAt)}</p>
                    {item.playerName ? <p className="mt-1 text-sm font-semibold leading-6 text-[#4b5f55]">Player: {item.playerName}</p> : null}
...
function QueueMetric({ label, value, tone = 'default' }) {
  const valueClass = tone === 'danger' ? 'text-rose-700' : 'text-[#101828]'
