  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-[max(5.5rem,env(safe-area-inset-bottom))] text-slate-950 lg:pb-0">
      ...
      <section className="mx-auto grid w-full max-w-7xl gap-6 px-4 py-8 pb-[max(2.5rem,env(safe-area-inset-bottom))] sm:px-6 sm:py-12 lg:grid-cols-[0.85fr_1fr] lg:gap-8 lg:px-8 lg:py-16">
        <div className="order-2 flex items-start justify-center overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-sm shadow-slate-200/80 sm:p-6 lg:sticky lg:top-28 lg:order-1 lg:max-h-[820px]">
          <img
        ...
        <div className="order-1 flex flex-col justify-center lg:order-2">
          ...
          <div className="mt-8 grid gap-4">
            {parentCards.map(([title, copy]) => (
              <article key={title} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm shadow-slate-200/80 sm:p-5">
