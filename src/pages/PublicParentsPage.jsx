...
const parentRules = [
  ...
  ['Football only', 'The family portal is built around players, teams, fixtures, reports, and parent actions.'],
]
      ...
      <section className="mx-auto grid w-full max-w-7xl gap-6 px-4 py-8 pb-[max(2.5rem,env(safe-area-inset-bottom))] sm:px-6 sm:py-12 lg:grid-cols-[0.85fr_1fr] lg:gap-8 lg:px-8 lg:py-16">
        ...
        <div className="order-1 flex flex-col justify-center lg:order-2">
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <a
              href="/parents/portal"
              className="inline-flex min-h-12 items-center justify-center rounded-lg bg-[#047857] px-5 py-3 text-sm font-black text-white transition hover:bg-[#065f46]"
            >
              Open family portal
            </a>
            <a
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            {parentCards.map(([title, copy]) => (
              <article key={title} className="rounded-lg border border-[#d7e5dc] bg-white p-4 shadow-sm shadow-[#047857]/10 sm:p-5">
                <h2 className="text-lg font-black sm:text-xl">{title}</h2>
                <p className="mt-2 text-sm font-semibold leading-7 text-[#4b5f55]">{copy}</p>
              </article>
            ))}
          </div>
          <section className="mt-5 rounded-lg border border-[#bbf7d0] bg-[#ecfdf5] p-5 shadow-sm shadow-[#047857]/10">
            <p className="text-xs font-black uppercase tracking-[0.16em] text-[#047857]">How it should feel</p>
