  ...
  return (
    <main className="min-h-screen bg-[#fbfdfb] pb-[max(5.5rem,env(safe-area-inset-bottom))] text-slate-950 lg:pb-0">
      ...
      <section className="mx-auto grid w-full max-w-7xl gap-6 px-4 py-8 pb-[max(2.5rem,env(safe-area-inset-bottom))] sm:px-6 sm:py-12 lg:grid-cols-[0.85fr_1fr] lg:gap-8 lg:px-8 lg:py-16">
        <div className="order-2 flex items-start justify-center overflow-hidden rounded-lg border border-slate-200 bg-white p-4 shadow-sm shadow-slate-200/80 sm:p-6 lg:sticky lg:top-28 lg:order-1 lg:max-h-[820px]">
          <img
        ...
        <div className="order-1 flex flex-col justify-center lg:order-2">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-700">Parents portal</p>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            ...
            <a
              href="/pricing"
              className="inline-flex min-h-12 items-center justify-center rounded-lg border border-slate-200 bg-white px-5 py-3 text-sm font-black text-slate-900 shadow-sm transition hover:bg-slate-50"
            >
              View Pricing
            </a>
          </div>
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            {parentCards.map(([title, copy]) => (
              <article key={title} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm shadow-slate-200/60 sm:p-5">
                <h2 className="text-lg font-black sm:text-xl">{title}</h2>
                <p className="mt-2 text-sm font-semibold leading-7 text-slate-600">{copy}</p>
              </article>
            ))}
          </div>
          <section className="mt-5 rounded-lg border border-emerald-200 bg-[#f2fbf6] p-5 shadow-sm shadow-emerald-100/60">
            <p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-700">How it should feel</p>
