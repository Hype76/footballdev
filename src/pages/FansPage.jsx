...
function FansWorkspace({ user, signOut, parentId, setParentId }) {
  ...
  return <FanBrandScope source={brandSource}><main className={`fans fans-workspace ${parents.length ? 'fans-managed' : 'fans-standalone'}`}>
    ...
    {section === 'players' && !state.view ? <section className="fans-players" aria-label="Players you follow">{parents.length ? <h2>Players you follow</h2> : null}
      ...
      <div className="fans-player-grid">{followed.map((c) => <FanBrandScope source={c} key={c.id}><article className="fans-following"><FanClubBrand source={c} /><div className="fans-player-identity"><span className="fans-player-avatar"><FanIcon /></span><div><h2>{c.player_name}</h2><small>{c.team_name}</small></div></div>
        <div className="fans-feature-grid">{FAN_ACCESS.filter((p) => c.permissions[p.key]).map((p) => <button key={p.key} onClick={() => state.open(c.id, p.key === 'game_day' ? 'matches' : p.key)}><FanIcon name={p.key} /><span>{p.key === 'resources' ? 'Resources' : p.label}</span><span aria-hidden="true" className="fans-chevron">›</span></button>)}</div>
        {c.permissions.game_day ? <div className="fans-notifications"><button onClick={() => state.open(c.id, 'notifications')}>View notifications <span aria-hidden="true">›</span></button><label><span>Game Day notifications</span><input aria-label={`Game Day notifications for ${c.player_name}`} role="switch" type="checkbox" disabled={busy} checked={Boolean(c.notifications_enabled)} onChange={(e) => run(() => state.manage(c.id, e.target.checked ? 'notifications_on' : 'notifications_off'))} /></label></div> : null}
