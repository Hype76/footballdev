...
function FansWorkspace({ user, signOut, parentId, setParentId }) {
  ...
  return <FanBrandScope source={brandSource}><main className="fans">
    ...
    {parents.length ? <section>
      <label>Child<select aria-label="Child" disabled={busy} value={selectedParent?.id || ''} onChange={(e) => { setParentId(e.target.value); state.clearView(); setForm(null); setReady(null); setQr('') }}>{parents.map((p) => <option value={p.id} key={p.id}>{p.playerName}</option>)}</select></label>
