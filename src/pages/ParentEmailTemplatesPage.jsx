...
import { EMAIL_TEMPLATE_FIELDS, validateParentEmailTemplateContent } from '../lib/email-templates.js'
import {
...
function mergeTemplates(savedTemplates) {
  const savedByKey = new Map(savedTemplates.map((template) => [template.key, template]))

  return getDefaultClubParentEmailTemplates().map((defaultTemplate) => ({
    ...defaultTemplate,
...
export function ParentEmailTemplatesPage() {
  const { user } = useAuth()
  const [templates, setTemplates] = useState(() => mergeTemplates([]))
  ...
  useEffect(() => {
    ...
    const loadTemplates = async () => {
      ...
      try {
        const savedTemplates = await getParentEmailTemplates({ user, includeDisabled: true })

        if (isMounted) {
          setTemplates(mergeTemplates(savedTemplates))
        }
      } catch (error) {
        console.error(error)
  ...
  const resetTemplate = (templateKey) => {
    const defaultTemplate = getDefaultClubParentEmailTemplates().find((template) => template.key === templateKey)
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      <PageHeader
        ...
        description="Create the club templates used when sending parent emails."
      />
      ...
      {isLoading ? (
        ...
        templates.map((template) => (
          <SectionCard
            ...
            description={template.id ? 'This saved club template is available when sending parent emails.' : 'Save this default before it can be used for parent emails.'}
          >
