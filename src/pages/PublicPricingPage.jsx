      ...
      <section className="mx-auto w-full max-w-7xl px-4 py-8 pb-[max(2.5rem,env(safe-area-inset-bottom))] sm:px-6 sm:py-12 lg:px-8 lg:py-16">
        <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/80 sm:p-6 lg:p-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            ...
            <p className="mt-5 text-base font-semibold leading-7 text-slate-700 sm:leading-8">
            ...
            <div className="grid w-full grid-cols-2 rounded-lg border border-slate-200 bg-[#f8fafc] p-1 sm:max-w-xs">
            {[
              ['monthly', 'Monthly'],
              ['annual', 'Annual'],
            ].map(([key, label]) => (
              <button
                ...
                className={[
                  ...
                  billingCycle === key ? 'bg-emerald-700 text-white shadow-sm' : 'text-slate-600 hover:bg-white hover:text-slate-950',
                ].join(' ')}
              >
                {label}
              </button>
        ...
        {message ? <div className="mt-6 rounded-lg border border-emerald-200 bg-[#f2fbf6] px-4 py-3 text-sm font-semibold text-emerald-800">{message}</div> : null}
        {livePromotion && !paymentsDisabled ? (
          <div className="mt-6 rounded-lg border border-emerald-200 bg-[#f2fbf6] px-5 py-4 text-sm font-bold text-emerald-800">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {pricingPlans.map((plan) => {
            ...
            return (
              <article key={plan.name} className="relative flex flex-col rounded-lg border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/70">
                {plan.name === 'Small Club' ? (
                  <span className="absolute right-4 top-4 rounded-lg border border-emerald-200 bg-[#f2fbf6] px-3 py-1 text-xs font-bold text-emerald-800 sm:right-5 sm:top-5">
                    Popular
                  </span>
                ) : null}
