...
test('attendance retains real times, Home/Away, scoped history and saved responses',async()=>{
 ...
 const client={from(table){let excluded=[];const query={then(resolve){return Promise.resolve({data:(tables[table]||[]).filter(r=>excluded.every(([k,v])=>r[k]!==v))}).then(resolve)},neq(k,v){excluded.push([k,v]);return query}};for(const method of ['select','eq','is','gte','order','limit','in','range'])query[method]=()=>query;return query}}
 const rows=await loadPlayerAttendance(client,{club:{name:'Club'},fan:{club_id:'club',relationship_type:'player',permissions:{schedule:true}},parent:{club_id:'club',team_id:'team'},player:{id:'player',team_id:'team'}},new Date('2026-09-17T12:00:00Z'))
