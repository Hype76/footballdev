...
test('fixture and squad modals use mobile-safe flex scroll layouts', async () => {
  ...
  assert.match(fixtureSection, /className="min-h-0 flex-1 overflow-y-auto overscroll-contain scroll-pb-40 scroll-pt-28/)
  assert.match(fixtureSection, /onFocusCapture=\{\(event\) => scrollFixtureControlIntoView\(event\.target\)\}/)
