...
test('quick actions expose Create Poll only to permitted staff', async () => {
  ...
  assert.match(source, /saveQuickActionPosition\(finalPosition\)/)
  assert.match(source, /suppressNextQuickActionClickRef\.current/)
...
test('fixture and squad modals use mobile-safe flex scroll layouts', async () => {
  ...
  assert.match(fixtureSection, /className="hidden shrink-0 flex-col gap-4 border-b[\s\S]*sm:flex/)
  assert.match(fixtureSection, /shouldPrioritizeFixtureFields \? 'hidden' : 'flex'[\s\S]*sm:hidden/)
  ...
  assert.match(squadSection, /className="hidden shrink-0 border-b[\s\S]*sm:block/)
  assert.match(squadSection, /className="shrink-0 border-b[\s\S]*sm:hidden/)
  assert.match(squadSection, /className="min-h-0 flex-1 overflow-y-auto overscroll-contain scroll-pb-32 scroll-pt-4/)
  assert.match(squadSection, /sm:scroll-pb-40/)
  assert.match(squadSection, /sm:scroll-pt-24/)
  assert.match(squadSection, /className="mb-3 flex flex-wrap[\s\S]*sm:hidden/)
  assert.match(squadSection, /className="mt-4 hidden flex-wrap[\s\S]*sm:flex/)
  assert.match(squadSection, /hidden text-sm font-semibold[\s\S]*sm:block/)
...
test('calendar event modal uses mobile keyboard-safe flex scroll layout', async () => {
  ...
  assert.match(modalSection, /top-\[var\(--calendar-modal-viewport-top\)\][\s\S]*h-\[var\(--calendar-modal-viewport-height\)\]/)
