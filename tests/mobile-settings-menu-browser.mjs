...
try {
  ...
  for (const app of ['parent', 'coach']) {
    ...
    if (app === 'parent') {
      await page.evaluate(() => window.override({ notificationStateStatus: 'stale' }))
      await page.getByText('Unable to verify', { exact: true }).waitFor()
      assert.equal(await page.getByText('Push alerts enabled', { exact: true }).count(), 0)
      await open('Retry notification check')
      assert.ok(await page.evaluate(() => window.calls.some(call => call.name === 'retry')))
      await open('Enable push alerts on this device')
      assert.ok(await page.evaluate(() => window.calls.some(call => call.name === 'push' && call.args[0] === 'minimal')))
      const enable = await page.getByRole('button', { name: 'Enable push alerts on this device', exact: true }).boundingBox()
      const categories = await page.getByText('Game Day', { exact: true }).boundingBox()
      assert.ok(enable.y < categories.y, 'Recovery is above category preferences')
      await page.getByRole('button', { name: 'Open device notification settings', exact: true }).waitFor()
      await page.screenshot({ path: `${out}/parent-notification-recovery.png`, fullPage: true })
    }
    await back()
