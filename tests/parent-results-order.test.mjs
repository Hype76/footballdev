...
test('Parent Portal applies the shared sorter after result filtering and keeps loading and isolation safeguards', async () => {
  ...
  assert.match(page, /let isCurrent = true[\s\S]*getParentPortalMatchDays\(\{ parentLinkId: selectedLink\.id \}\), setMatches/)
  assert.match(page, /const value = await read\(\)\s*if \(isCurrent\) publish\(value\)/)
