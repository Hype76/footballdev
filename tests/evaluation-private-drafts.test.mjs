...
test('private draft autosave detects offline state before queueing one recoverable server save', () => {
  ...
  assert.match(source, /isSamePrivateEvaluationDraftSave\(inFlightSave\.active, stagedSave\)/)
  assert.match(source, /isSamePrivateEvaluationDraftSave\(inFlightSave\.pending, stagedSave\)/)
...
test('request coordinator rejects out-of-order, navigation, and form-change responses', () => {
  ...
  const teamBRevision = coordinator.nextRequest(teamBPlayerAFormA)
