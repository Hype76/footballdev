...
test('staff Match Day page renders a staff-only match timeline from existing match events', async () => {
  ...
  assert.notEqual(panelStart, -1)
  assert.notEqual(panelEnd, -1)
  assert.match(source, /<MatchTimelinePanel events=\{events\} \/>/)
  assert.match(source, /function getMatchEventTypeLabel\(event\)/)
  ...
  assert.match(source, /event\.teamSide === 'opponent' \? 'Opponent goal' : 'Our goal'/)
  ...
  assert.match(panelSource, /No match timeline events yet\./)
  assert.match(panelSource, /Goals and score corrections will appear here when they are recorded\./)
  assert.match(panelSource, /const timelineEvents = Array\.isArray\(events\) \? events : \[\]/)
  assert.match(panelSource, /const recentEvents = timelineEvents\.slice\(0, 8\)/)
  assert.doesNotMatch(panelSource, /fetch\(/)
  assert.doesNotMatch(panelSource, /createMatchDayEventLogEntry/)
  assert.doesNotMatch(panelSource, /sendMatchDayPushNotification/)
  assert.doesNotMatch(panelSource, /scheduled_email_queue/)
...
test('staff add goal panel previews score impact and fallback details without new write paths', async () => {
  ...
  const formEnd = source.indexOf('<MatchTimelinePanel events={events} />', formStart)
  ...
  assert.match(formSource, /Result if saved: \{goalPreview\.scoreAfter\}/)
  assert.match(formSource, /Goal side/)
  assert.match(formSource, /\{ label: 'Our team', value: 'club' \}/)
  assert.match(formSource, /\{ label: 'Opponent', value: 'opponent' \}/)
  assert.match(formSource, /aria-pressed=\{isSelected\}/)
  assert.match(formSource, /Score preview/)
  assert.match(formSource, /Home after save: \{goalPreview\.homeScore\}\. Away after save: \{goalPreview\.awayScore\}\./)
  assert.match(formSource, /value=\{goalForm\.minute\}/)
  assert.match(formSource, /value=\{goalForm\.notes\}/)
  assert.match(formSource, /\['Scorer', goalPreview\.scorerPreview\]/)
  assert.match(formSource, /\['Assist', goalPreview\.assistPreview\]/)
  assert.match(formSource, /\['Minute', goalPreview\.minutePreview\]/)
  assert.match(formSource, /\['Note', goalPreview\.notePreview\]/)
  assert.match(handlerSource, /const formGoal = goalForms\[match\.id\] \?\? EMPTY_GOAL_FORM/)
  assert.match(handlerSource, /minute: normalizeStaffGoalText\(formGoal\.minute\) \|\| \(getCurrentMatchMinute\(match, Date\.now\(\)\) \?\? ''\)/)
...
test('parent portal does not expose the staff event log in this batch', async () => {
  ...
  assert.doesNotMatch(parentSource, /No match timeline events yet\./)
  ...
  assert.match(staffSource, /Result if saved/)
