...
test('Game Mode exposes minimal live controls and keeps full management separate', async () => {
  ...
  assert.match(source, /const normalTimeCompletionAction = getNormalTimeCompletionAction\(match\)/)
  assert.match(source, /onStatusChange\(match, normalTimeCompletionAction\)/)
  assert.match(source, /normalTimeCompletionAction === 'full_time' \? <span>FT<\/span> : <span>End normal time<\/span>/)
