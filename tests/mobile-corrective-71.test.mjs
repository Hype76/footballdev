...
test('corrective 71 freezes guarded Coach and Parent production candidates', async () => {
  ...
  assert.match(coachConfig, /version: '1\.0\.2[1-9]'/)
  assert.ok(Number(JSON.parse(coachPackage).version.split('.')[2]) >= 21)
  assert.match(parentConfig, /version: '1\.0\.(?:1[89]|2[0-9])'/)
  assert.ok(Number(JSON.parse(parentPackage).version.split('.')[2]) >= 18)
