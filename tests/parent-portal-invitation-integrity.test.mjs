...
test('Parent Calendar modal and Invites use the same invitation state and refresh after mutation', async () => {
  ...
  assert.match(modal, /Player availability/)
  ...
  assert.match(invites, /aria-label="Invitation pages"/)
  assert.match(invites, /nextSearchParams\.set\('inviteView', viewId\)/)
  assert.match(invites, /nextSearchParams\.set\('invitePage', String\(nextPage\)\)/)
