...
test('calendar UI attaches resources only to team custom events and protects recurring edits', async () => {
  ...
  assert.match(source, /getCalendarEventResources\(\{ user, eventId, occurrenceDate, teamId: eventTeamId \}\)/)
  assert.match(source, /syncCalendarEventResourceLinks\({[\s\S]*eventId: savedEvent\.id,[\s\S]*occurrenceDate: resourceOccurrenceDate,[\s\S]*teamId: safeTeamId,[\s\S]*resourceIds: calendarForm\.resourceIds/)
  assert.match(source, /Attached resources/)
  assert.match(source, /onClick=\{\(\) => onOpenResource\?\.\(resource\)\}/)
  assert.match(source, /Open attachment/)
  assert.match(source, /getResourceLibraryDownloadUrl\(\{[\s\S]*resourceId: resource\.id,[\s\S]*teamId: safeFormTeamId/)
