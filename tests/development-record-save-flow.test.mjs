...
test('submit lock prevents repeated clicks from duplicating core or optional output work', async () => {
  const pageSource = await readFile(new URL('../src/pages/CreateEvaluationPage.jsx', import.meta.url), 'utf8')
  const sectionSource = await readFile(
    new URL('../src/components/evaluations/SubmitExportSection.jsx', import.meta.url),
    'utf8',
  )

  assert.match(pageSource, /const submitLockRef = useRef\(false\)/)
  assert.match(pageSource, /if \(submitLockRef\.current\) \{\s*return\s*\}/)
  assert.match(pageSource, /submitLockRef\.current = true[\s\S]+setIsSubmitting\(true\)/)
  assert.match(pageSource, /finally \{[\s\S]+submitLockRef\.current = false/)
  assert.match(sectionSource, /disabled=\{isSubmitting \|\| !canSubmitEvaluation\}/)
})

test('optional email and PDF work starts only after the core evaluation is saved', async () => {
  const source = await readFile(new URL('../src/pages/CreateEvaluationPage.jsx', import.meta.url), 'utf8')
  const saveStart = source.indexOf('const savedEvaluation = editingEvaluation')
  const outputStart = source.indexOf("if (previewMode === 'email' && selectedParentEmail)", saveStart)
  const outputEnd = source.indexOf('const savedPlayerId =', outputStart)
  const outputSource = source.slice(outputStart, outputEnd)

  assert.notEqual(saveStart, -1)
  assert.ok(outputStart > saveStart)
  assert.match(outputSource, /attachPdf: isPdfAttachmentApproved/)
  assert.match(outputSource, /await Promise\.all\(emailJobs\.map/)
  assert.match(outputSource, /completionOutcome = isScheduledSend \? 'scheduled' : 'sent'/)
  assert.match(outputSource, /catch \(emailError\)[\s\S]+completionOutcome = emailSendMode === 'scheduled' \? 'schedule_failed' : 'send_failed'/)
  assert.doesNotMatch(outputSource, /throw emailError/)
})

test('optional output failure messaging confirms the core Development Record is safe', async () => {
  const source = await readFile(new URL('../src/pages/CreateEvaluationPage.jsx', import.meta.url), 'utf8')

  assert.match(
    source,
    /development record has been saved, but the parent email could not be sent\./,
  )
  assert.match(
    source,
    /development record has been saved, but the parent email could not be scheduled\./,
  )
  assert.match(source, /setCompletionModal\(getCompletionModalForOutcome\(/)
})
