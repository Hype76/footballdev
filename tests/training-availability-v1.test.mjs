...
test('training availability chip state mapping covers accepted, unavailable, pending, and maybe', async () => {
  ...
  assert.deepEqual(getTrainingAvailabilityChipState('available'), { label: 'Attending', tone: 'green' })
  assert.deepEqual(getTrainingAvailabilityChipState('accepted'), { label: 'Attending', tone: 'green' })
  assert.deepEqual(getTrainingAvailabilityChipState('not accepted'), { label: 'Not attending', tone: 'red' })
  assert.deepEqual(getTrainingAvailabilityChipState('unavailable'), { label: 'Not attending', tone: 'red' })
...
test('scheduled processor claims bounded due work without push, sms, or volunteer roles', async () => {
  ...
  assert.match(processor, /invite\.training_availability_requested === true[\s\S]*invite\.notify_requested === true[\s\S]*response_requirement\) === 'response_required'/)
  ...
  assert.match(processor, /resolveEligibleEventInvitationContacts\(supabase, \{[\s\S]*clubId: request\.club_id[\s\S]*playerIds[\s\S]*teamId: request\.team_id/)
  assert.doesNotMatch(processor, /from\('parent_player_links'\)[\s\S]*\.in\('player_id', playerIds\)/)
