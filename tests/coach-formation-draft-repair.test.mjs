...
test('screen recovery keeps explicit team save, guarded replacement, and automatic device drafts', async () => {
  ...
  assert.match(screen, /saveCoachMatchFormationBoard\(user, match, nextBoard, saveDraft, saveTitle, saveShared\)/)
  ...
  assert.match(screen, /createdByProfileId: normalize\(previousPendingSave\?\.createdByProfileId\) \|\| normalize\(saveBoard\?\.createdByProfileId\) \|\| user.id/)
