...
try {
  ...
  await desktop.page.getByRole('heading', { name: 'Create session' }).waitFor({ state: 'visible', timeout: 15000 })
  ...
  assert.equal(await desktop.page.getByLabel('Share with parents?').count(), 1)
