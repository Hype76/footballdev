...
try {
  ...
  assert.equal(await expiredSelectedMenu.getByRole('menuitem', { name: 'Mark available on behalf' }).count(), 1)
  ...
  const sendsBeforeUnsentAcceptance = invitationActionCount
  await shortManager.locator('[role="row"][data-player-id="bulk-player-32"]').getByRole('button', { name: 'Expand' }).click()
  const unsentDetails = shortManager.locator('[role="row"][data-player-id="bulk-player-32-details"]')
  await unsentDetails.getByRole('button', { name: 'Actions for Long List Player 32' }).click()
  await unsentDetails.getByRole('menuitem', { name: 'Mark available on behalf' }).click()
  await shortAndroid.page.getByRole('dialog').last().getByRole('button', { name: 'Accept on behalf of player' }).click()
  await shortManager.getByRole('tab', { name: 'Available (3)' }).waitFor({ state: 'visible', timeout: 15000 })
  assert.equal(invitationActionCount, sendsBeforeUnsentAcceptance)
  assert.equal(matchState.availability.find((row) => row.player_id === 'bulk-player-32')?.status, 'available')
  assert.equal(matchState.requests.some((row) => row.player_id === 'bulk-player-32'), false)
