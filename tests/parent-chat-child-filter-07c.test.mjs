...
test('Parent UI exposes an accessible default-off switch only after server eligibility', async () => {
  ...
  assert.match(workspace, />\s*Your player only\s*</)
  assert.match(workspace, /variant === 'parent' && childFilterAvailable[\s\S]*role="switch"[\s\S]*Your player only/)
  assert.match(shell, /onSelectedParentLinkChange/)
...
test('scope changes and slow responses cannot restore stale room metadata or duplicate subscriptions', async () => {
  ...
  assert.match(workspace, /const unsubscribe = subscribeToParentChatRoom/)
  assert.match(workspace, /return \(\) => \{\s*active = false\s*unsubscribe\(\)/)
  assert.match(workspace, /window\.clearInterval\(timer\)/)
