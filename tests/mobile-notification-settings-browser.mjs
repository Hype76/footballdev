...
try {
  ...
  for(const app of ['coach','parent']) {
    ...
    assert.equal(await scores.getAttribute('aria-checked'),'true')
    for(const name of ['Invites','Chats','New resources']) assert.equal(await page.getByRole('switch',{name,exact:true}).isChecked(),true)
    await page.getByRole('radio',{name:'Off',exact:true}).click()
    await page.getByText('Saved.',{exact:true}).waitFor()
    assert.equal(await page.getByRole('switch',{name:'Invites',exact:true}).isChecked(),true)
    ...
    await page.getByRole('switch',{name:'Chats',exact:true}).click()
    await page.waitForFunction(()=>!document.body.innerText.includes('Saving your choice'))
    assert.equal(await page.getByRole('switch',{name:'Chats',exact:true}).isChecked(),false)
  ...
  await mount()
  await page.waitForFunction(()=>!document.body.innerText.includes('Checking your saved choices'))
  assert.equal(await page.getByRole('radio',{name:'Full Game Day notifications',exact:true}).getAttribute('aria-checked'),'true')
  assert.equal(await page.getByRole('switch',{name:'Chats',exact:true}).isChecked(),false)
  await page.evaluate(()=>window.switchUser('two'))
  await page.waitForFunction(()=>!document.body.innerText.includes('Checking your saved choices'))
  assert.equal(await page.getByRole('radio',{name:'Score and cards only',exact:true}).getAttribute('aria-checked'),'true')
  ...
  assert.equal(await page.getByRole('switch',{name:'Invites',exact:true}).isDisabled(),true)
  assert.equal(await page.getByRole('radio',{name:'Off',exact:true}).getAttribute('aria-checked'),'false')
