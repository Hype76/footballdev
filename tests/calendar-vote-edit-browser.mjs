...
try {
  ...
  await vote.check()
  const expiry=page.getByLabel('Vote expiry (hours)')
  assert.equal(await expiry.inputValue(),'6','Existing expiry is preserved')
  ...
  assert.equal(await page.evaluate(()=>window.saved.motmPollExpiryHours),'12')
  await page.evaluate(()=>window.reopen())
  assert.equal(await vote.isChecked(),true)
  assert.equal(await expiry.inputValue(),'12')
  ...
  await vote.uncheck()
  assert.equal(await page.getByLabel('Vote expiry (hours)').count(),0)
