...
function ParentHero({mode,pitch,fixtureType,homeAway}){const{colors,styles}=usePortalStyles(createParentMobileTheme({mode}).tokens),selectedMatch=normalizeMatchDay({id:'match',opponent:'Visitors',teamName:'FP TEST',pitch_type:pitch,fixture_type:fixtureType,match_date:'2099-09-12',kickoff_time:'11:00:00',arrival_time:'10:00:00',venue_name:'FP TEST ground',home_away:homeAway}),selectedMatchIsLive=false,presentation=null,now=Date.now();window.normalized=selectedMatch;return ${hero}}
...
return <View style={{backgroundColor:palette.background,minHeight:'100vh',padding:12}}>{screen==='home'?<HomeScreen user={user} context={context} homeState={{matches:[],sessions:[],nextCalendar:chosen}} onNavigate={(route,t)=>{window.calls.push({route,target:t});setTarget(t);setScreen('calendar')}} reloadHome={()=>{}}/>:screen==='calendar'?<CoachCalendarScreen key={key} calendarTarget={target} context={context} contexts={contexts} user={user} palette={palette} onNavigate={()=>{}}/>:<ParentHero mode={mode} pitch={pitch} fixtureType={fixtureType} homeAway={homeAway}/>}</View>};createRoot(document.getElementById('root')).render(<App/>);`
...
try {
 ...
 for(const [pitch,label] of [['grass','Grass'],['3g','3G'],['4g','4G'],['astro','Astro'],['indoor','Indoor'],['other','Other'],['','Not specified']]){await page.evaluate(v=>window.pitch(v),pitch);await page.getByText('Surface: '+label,{exact:true}).waitFor();assert.equal(await page.evaluate(()=>window.normalized.pitchType),pitch);assert.equal(await page.locator('[data-testid^="pitch-type-"]').count(),pitch?1:0)}
 for(const [type,label] of [['league','League'],['cup','Cup'],['tournament','Tournament'],['friendly','Friendly'],[' CUP ','Cup'],['match','Not specified'],['','Not specified'],['unknown','Not specified']]){await page.evaluate(v=>window.fixtureType(v),type);await page.getByText('Match type: '+label,{exact:true}).waitFor();assert.equal(await page.locator('[data-testid^="match-type-"]').count(),label==='Not specified'?0:1)}
 for(const [venue,label] of [['home','Home game'],['away','Away game'],[' HOME ','Home game'],['neutral','Neutral venue'],['unknown','Venue type not specified']]){await page.evaluate(v=>window.homeAway(v),venue);await page.getByText(label,{exact:true}).waitFor();assert.equal(await page.locator('[data-testid^="home-away-"]').count(),label.endsWith('game')?1:0)}
...
for(const mode of ['light','dark'])for(const venue of ['home','away']){await page.evaluate(v=>{window.mode(v.mode);window.homeAway(v.venue)},{mode,venue});await page.getByTestId('home-away-'+venue).waitFor();await page.screenshot({path:out+'/venue-'+venue+'-'+mode+'.png',fullPage:true})}
