...
test('Coach checks the production update channel on launch', () => {
  ...
  assert.match(app, new RegExp(`version: '${packageVersion.replaceAll('.', '\\.')}'`))
  assert.match(packageJson, /"expo-updates": "~29\.0\.20"/)
...
test('Coach session refresh follows app foreground state and store guards stay explicit', () => {
  ...
  assert.match(buildGuard, /FP-MOBILE-COACH-FORM-SELECTION-CORRECTIVE-54/)
  assert.match(submitGuard, /FP-MOBILE-COACH-FORM-SELECTION-CORRECTIVE-54/)
})
