...
const loginCopy=coach
 ? {title:'Your team. Your match day.',kicker:'Football Player Coach',copy:'Use the same active Coach account you use on the website.',emailPlaceholder:'coach@example.test',meta:'Restricted club access.'}
 : {title:'Everything for your player, in one place.',kicker:'Parent access',copy:'Use the email and password linked to your family account.',emailPlaceholder:'parent@example.test',meta:'Private family access. Password sign-in only.'}
...
createRoot(document.getElementById('root')).render(<MobileLoginScreen {...${JSON.stringify(loginCopy)}} logoSource={{uri:'data:image/svg+xml,<svg xmlns=\"http://www.w3.org/2000/svg\"/>'}} signIn={async(...args)=>window.logins.push(args)} requestPasswordReset={async(email)=>window.resets.push(email)}/>);
...
try {
 ...
 const email=page.getByPlaceholder(loginCopy.emailPlaceholder),password=page.getByPlaceholder('Password',{exact:true})
 ...
 await page.screenshot({path:`${out}/${coach?'coach':'parent'}-password-reduced-viewport.png`})
