...
test('Resource Library UI keeps V1 staff scope and uses explicit parent sharing', async () => {
  ...
  assert.doesNotMatch(page, /parentVisible \? 'Shared with parents' : 'Staff only'/)
  ...
  assert.match(page, /createAssignmentDraftForResource/)
  assert.match(page, /replacePlayerAssignments: isSquadAssignment \|\| isPlayerAssignment/)
  assert.match(page, /Edit player assignments/)
