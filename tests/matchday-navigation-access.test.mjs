...
test('match day domain scopes reads and mutations to the active team', async () => {
  ...
  assert.match(goalSection, /supabase\.rpc\('record_match_day_goal_v2'/)
  assert.match(goalSection, /match_day_id_value: match\.id/)
  assert.match(goalSection, /request_id_value: createMatchDayRequestId\(requestId\)/)
  assert.doesNotMatch(goalSection, /\.from\('match_days'\)[\s\S]*\.update\(/)
