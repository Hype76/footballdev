...
test('domain wrappers call correction RPCs without direct event table updates', async () => {
  ...
  assert.match(domain, /export async function voidStaffMatchDayEvent/)
  ...
  assert.match(correctionSource, /supabase\.rpc\('void_match_day_event'/)
...
test('staff and scorer UI expose correction controls only through RPC handlers', async () => {
  ...
  assert.doesNotMatch(parentPage, /await voidMatchDayGoalAsScorer\(/)
  ...
  assert.doesNotMatch(parentPage, /onVoidGoal|handleVoidGoal|Undo event/)
