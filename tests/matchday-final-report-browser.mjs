...
async function run() {
  ...
  try {
    ...
    await staff.page.getByRole('heading', { name: 'Active fixtures' }).waitFor({ state: 'visible' })
    await staff.page.getByRole('button', { name: /Previous games/ }).click()
    await staff.page.getByRole('button', { name: /Manage City Juniors v U12 Fixture Team/ }).click()
    ...
    await goalsAndCardsButton.click()
    ...
    await report.getByText('1 yellow, 0 red', { exact: true }).last().waitFor({ state: 'visible' })
