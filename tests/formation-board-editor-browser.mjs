...
try {
  ...
  await desktopPage.getByRole('button', { name: 'Take Players off lineup', exact: true }).click()
  ...
  assert.equal(await desktopPage.locator('[data-formation-pitch]').getByRole('button').count(), 1)
  await desktopPage.getByRole('button', { name: 'Undo', exact: true }).click()
  assert.equal(await desktopPage.locator('[data-formation-pitch]').getByRole('button').count(), 3)
  ...
  for (let index = 1; index <= 5; index += 1) {
    const name = index === 1 ? 'Synthetic One' : `Synthetic Player ${index}`
    await capacityTray.getByRole('button', { name: new RegExp(`${name}.*Bench`) }).click()
  ...
  await capacityPage.getByText('This 5v5 pitch already has 5 Players. Move a Player to the Bench first.', { exact: true }).waitFor()
  assert.equal(await capacityPitch.getByRole('button').count(), 5)
  ...
  await capacityPage.getByLabel('Game format').selectOption('5v5')
  await capacityPage.getByRole('button', { name: 'Change formation', exact: true }).click()
  await capacityPage.getByText('1 excess Player was moved to the Bench. No Player was dropped, and the change is available in Undo.', { exact: true }).waitFor()
  assert.equal(await capacityPitch.getByRole('button').count(), 5)
