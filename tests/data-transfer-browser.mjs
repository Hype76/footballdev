...
async function prepareContext(browser, workbookBuffer, options = {}) {
  ...
  await context.route('**/.netlify/functions/data-transfer', async (route) => {
    ...
    if (body.operation === 'blank' || body.operation === 'export') {
    ...
    if (body.operation === 'inspect') {
      ...
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ success: true, batch: { id: 'batch-fixture', state: 'ready_for_review', workbookSha256: 'a'.repeat(64), templateVersion: 'FP-V1-ONBOARDING-1', counts: { total: 5, create: 3, update: 1, skip: 1 } }, confirmationToken: 'confirm-fixture', errors: [], warnings: [], preview: [{ sheet: 'Players', row: 2, entityType: 'player', reference: 'PLAYER-1', outcome: 'create', codes: [], explanation: 'Create player.', proposedChanges: { first_name: 'Alex' } }] }) })
      return
    }
  ...
  return { context, blankCalls: () => blankCalls, confirmCalls: () => confirmCalls, inspectedTeamIds: () => inspectedTeamIds, rawCalls: () => rawCalls }
}
...
try {
...

  {
    ...
    assert.equal(await page.getByLabel('Allow authorized missing teams to be created in club-wide scope').isDisabled(), true)
    await page.getByText('Fill approved blank platform fields').click()
    await page.getByText('Use reviewed workbook values for populated field conflicts').click()
    ...
    await page.getByRole('button', { name: 'Inspect workbook' }).click()
    ...
    await page.getByText('I reviewed the scope and row-level preview. I understand guardian invitations are not sent.').click()
    ...
    await page.screenshot({ path: 'outputs/fp-v1-data-transfer-batch1-implement-01/data-transfer-page.png', fullPage: true })
  ...
  for (const [label, viewport] of [['tablet', { width: 820, height: 1180 }], ['mobile', { width: 390, height: 844 }]]) {
    ...
    await page.goto(`${baseUrl}/data-transfer`, { waitUntil: 'domcontentloaded' })
    await page.getByRole('heading', { name: 'Data Transfer' }).waitFor()
    await page.getByRole('button', { name: 'Download blank template' }).waitFor()
