...
test('follow-up queues a targeted staff message without modifying any availability response; retry is idempotent', async () => {
  ...
  assert.equal(row.payload.communicationLog.metadata.type, 'availability_follow_up')
  assert.equal(row.payload.communicationLog.metadata.eventTitle, 'Visitors')
  assert.match(row.payload.communicationLog.metadata.eventDateLabel, /01:01:2099/)
  assert.match(row.payload.resendPayload.html, /Visitors/)
  assert.match(row.payload.resendPayload.html, /01:01:2099/)
