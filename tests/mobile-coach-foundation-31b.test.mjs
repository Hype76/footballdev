...
test('role-aware navigation exposes no dead Team routes or Platform Admin governance', () => {
  ...
  assert.equal(coachNavigation.more.some((route) => route.key === 'notifications'), true)
  assert.deepEqual(getCoachRouteState('notifications'), { activeRoute: 'more', moreRoute: 'notifications' })
