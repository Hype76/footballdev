...
function fixture() {
  return {
    ...
    players: [
      { id: 'player-1', club_id: 'club-qa', team_id: 'team-u12', first_name: 'Zo?', last_name: 'Example', preferred_name: 'Z', date_of_birth: '2014-01-20', gender: 'Female', section: 'Squad', shirt_number: '007', positions: ['Defender', 'Midfielder'], status: 'active' },
...
test('ordinary export rows enforce team, club, season, status, and guardian visibility', () => {
  ...
  assert.deepEqual(players.map((row) => row.player_first_name), ['Zo?'])
...
test('CSV, XLSX, and ODS ordinary player and parent exports safely reimport through Phase 1 mapping', async () => {
  ...
  for (const format of ['csv', 'xlsx', 'ods']) {
    ...
    assert.equal(mapped.rowsBySheet.Players[0].first_name, 'Zo?')
  }
})
