...
import {getMatchDayDisplayName} from './src/lib/matchday-display.js';const getCoachInviteHistory=async()=>({firstEmailSentAt:'2026-09-10T17:46:00Z',emailSends:3,resendRequests:2,recentEmailSends:['2026-09-15T10:00:00Z'],recentResendRequests:[{at:'2026-09-17T07:09:00Z',queued:2,failed:0},{at:'2026-09-15T12:38:00Z',queued:2,failed:0}]});window.availabilityWrites=[];window.coachAttendanceWrites=[];const setCoachInviteAvailabilityOnBehalf=async(u,invite,status)=>{if(window.failAvailability)throw new Error('Synthetic save failed');window.availabilityWrites.push({id:invite.playerId,status});return{changed:true}};const submitOwnTrainingCoachAttendance=async(u,attendance,status)=>{window.coachAttendanceWrites.push({id:attendance.id,status});return{changed:true,status}};
...
const contentScrollRef=useRef(null),pendingScrollRestoreRef=useRef(null);const scrollBounds=useMemo(()=>createCoachScrollBounds(options=>contentScrollRef.current?.scrollTo(options)),[]);const activeContext={id:contextId},user={id:'coach-one',activeTeamId:'team',roleRank:rank};${scrollLifecycle}
...
try{
 ...
 await page.getByRole('button',{name:'Kyle De Dominicis invitation details'}).click();await page.getByText('Sent: 10:09:2026 18:46',{exact:true}).waitFor();await page.getByText('Reminder 1: 15:09:2026 13:38 (requested)',{exact:true}).waitFor();await page.getByText('Reminder 2: 17:09:2026 08:09 (requested)',{exact:true}).waitFor();await page.waitForTimeout(400);await page.screenshot({path:'output/playwright/coach-invite-history-simple.png',fullPage:true});await page.getByRole('button',{name:'Close invitation details'}).click()
 ...
 if(await page.getByRole('button',{name:'All 17',exact:true}).count())await page.getByRole('button',{name:'All 17',exact:true}).click();await page.getByRole('button',{name:'Ethan Clarke invitation details'}).click();await page.getByText('Sent: 10:09:2026 18:46',{exact:true}).waitFor();await page.getByText('Reminder 1: 15:09:2026 13:38 (requested)',{exact:true}).waitFor();await page.getByText('Reminder 2: 17:09:2026 08:09 (requested)',{exact:true}).waitFor();await page.getByRole('button',{name:'Close invitation details'}).click()
 ...
 await page.getByRole('heading',{name:'Coaches (3)'}).waitFor();await page.getByRole('heading',{name:'Players (17)'}).waitFor()
 await page.getByRole('button',{name:'Dave Miller attendance details'}).click();await page.getByRole('button',{name:'Not attending',exact:true}).click();await page.waitForFunction(()=>window.coachAttendanceWrites.length===1);assert.deepEqual(await page.evaluate(()=>window.coachAttendanceWrites),[{id:'coach-attendance-one',status:'unavailable'}])
