...
test('confirmation function renders a form and submits availability plus role replies', async () => {
  ...
  assert.match(source, /Current availability: \$\{escapeHtml\(sentenceStatusLabel\(currentStatus\)\)\}\. Last updated at \$\{escapeHtml\(selectedAt\)\} by \$\{escapeHtml\(selectedBy\)\}/)
  assert.match(source, /normalizeText\(response\.current_availability_selected_by_name\)/)
  assert.doesNotMatch(source, /response\.current_availability_selected_by_email/)
  assert.match(source, /formatReadableTimestamp\(\s*response\.current_availability_selected_at,\s*response\.club_timezone \|\| response\.timezone_name,\s*\)/)
...
test('confirmation function renders Add to calendar near fixture details without maps or volunteer dependency', async () => {
  ...
  assert.match(calendarSource, /ctz: resolveClubTimeZone\(response\.club_timezone \|\| response\.timezone_name\)/)
...
test('confirmation function captures structured transport response without free text notes', async () => {
  ...
  assert.match(source, /Coaches coordinate transport manually\. These answers are not shared with other parents\./)
...
test('domain normalizer exposes staff and parent response fields', async () => {
  ...
  const performanceReadMigration = await readFile(performanceReadMigrationUrl, 'utf8')
  ...
  assert.match(performanceReadMigration, /'match_day_player_availability'/)
  assert.match(performanceReadMigration, /'match_day_player_availability_history'/)
  assert.match(performanceReadMigration, /'match_day_role_assignments'[\s\S]*'email', parent_link\.email[\s\S]*'auth_user_id', parent_link\.auth_user_id/)
  assert.match(performanceReadMigration, /'match_day_availability_requests'[\s\S]*'players'[\s\S]*'parent_player_links'/)
...
test('staff transport risk summary is staff only and availability derived', async () => {
  ...
  assert.match(panelSource, /Coaches coordinate manually/)
  assert.match(panelSource, /Needs Coach follow-up/)
  ...
  assert.doesNotMatch(parentSource, /Needs Coach follow-up/)
...
test('staff transport coordination summary stays staff only and manually derived', async () => {
  ...
  assert.match(panelSource, /Coach-only/)
  ...
  assert.match(panelSource, /Coach chase list/)
  ...
  assert.doesNotMatch(parentSource, /Coach chase list/)
  assert.match(confirmSource, /Coaches coordinate transport manually\. These answers are not shared with other parents\./)
...
test('staff volunteer selection is resolved server side and queues notifications', async () => {
  ...
  assert.match(source, /adminSupabase\.rpc\('sync_match_day_scorer_assignment'/)
  assert.match(source, /selected_value: false/)
  assert.match(source, /selected_value: true/)
  assert.doesNotMatch(source, /function syncLegacyScorerAssignment/)
  assert.match(source, /\.from\('scheduled_email_queue'\)[\s\S]*\.insert\(/)
