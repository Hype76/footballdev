...
test('recoverable parent access states preserve the authenticated session', async () => {
  ...
  assert.match(section, /A temporary Parent-link lookup problem is not treated as proof that the link is missing/)
  assert.match(section, /await selectAccessMode\(accessMode, \{/)
  assert.match(section, /redirectTo: buildMainAppUrl/)
  assert.match(section, /Open \$\{option\.label\}/)
  assert.match(section, /Retry/)
  assert.doesNotMatch(section, /signOut/)
  assert.match(source, /accessRouteMismatch\?\.parentAccessUnavailable/)
  assert.match(source, /accessRouteMismatch\.parentAccessReason === 'lookup_failed' \|\| accessModeOptions\.length > 0/)
  assert.match(source, /<RecoverableParentAccessState/)
...
test('active parent-player link resolves to parent portal profile without app user row', async () => {
  ...
  assert.match(section, /const parentMemberships = isDemoAuthUser[\s\S]*await getParentPortalMemberships\(authUser\)/)
  assert.match(section, /const parentLinks = parentMemberships\.links/)
  assert.match(section, /const parentLinkLookupFailed = Boolean\(parentMemberships\.lookupFailed\)/)
...
test('dual access parent mode carries a safe route back to team access', async () => {
  ...
  assert.match(section, /parentAccessReason: parentLinkLookupFailed \? 'lookup_failed' : 'no_active_parent_link'/)
  assert.match(section, /accessModeOptions: buildParentAccessModeOptions\(\{ hasTeamAccess: true \}\)/)
  assert.match(section, /parentPortalLinks: parentLinks/)
})

test('confirmed login-intent mismatch stays strict while lookup failure remains recoverable', async () => {
  const profileSource = await readFile(parentProfileSourceUrl, 'utf8')
  const authSource = await readFile(authSourceUrl, 'utf8')
  const profileStart = profileSource.indexOf('export async function fetchUserProfile')
  const profileEnd = profileSource.indexOf('export async function updatePassword', profileStart)
  const profileSection = profileSource.slice(profileStart, profileEnd)
  const selectStart = authSource.indexOf('const selectAccessMode = async')
  const selectEnd = authSource.indexOf('const selectTeam = async', selectStart)
  const selectSection = authSource.slice(selectStart, selectEnd)

  assert.match(profileSection, /loginAccessIntent === 'parent'[\s\S]*!parentLinkLookupFailed[\s\S]*loginIntentMismatch: true/)
  assert.match(profileSection, /const hasTeamAccess = selectedAccessMode === 'parent'[\s\S]*canSwitchParentToStaff/)
  assert.match(selectSection, /isParentPortalUser\(userRef\.current\) \|\| accessRouteMismatch\?\.parentAccessUnavailable/)
  assert.match(selectSection, /requireExistingStaffAccess: isParentToStaffSwitch/)
  assert.match(selectSection, /if \(options\.redirectTo\) \{\s*window\.location\.assign\(options\.redirectTo\)/)
})
