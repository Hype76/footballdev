...
test('account unavailable copy is production-safe for live parent portal users', async () => {
  ...
  assert.match(section, /this parent portal path needs an active linked parent profile/)
  assert.match(section, /this parent portal view cannot find an active linked parent profile/)
  assert.match(section, /You can open your team workspace now/)
  assert.match(section, /same email address that received/)
  assert.match(section, /<details/)
  assert.match(section, /Next step/)
  assert.match(section, /What this means/)
  assert.match(section, /const canOpenTeamWorkspace = Array\.isArray\(accessModeOptions\)/)
  assert.match(section, /await selectAccessMode\('team'\)/)
  assert.match(section, /buildMainAppUrl\(TEAM_WORKSPACE_HOME_PATH\)/)
  assert.match(section, /Open team workspace/)
