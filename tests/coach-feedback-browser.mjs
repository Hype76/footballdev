...
const match={id:'test',teamName:'Our team',opponent:'Visitors',status,timerStatus:status==='scheduled'?'not_started':status==='live'?'running':status==='half_time'?'half_time':'full_time',homeScore:0,awayScore:0,matchDate:'2026-09-14',matchDurationMinutes:60,halfDurationMinutes:30};
return <View style={{backgroundColor:palette.background,padding:16,minHeight:'100vh'}}>{history?<CoachNotificationHistoryScreen user={{id:'test'}} context={{id:'test'}} palette={palette} styles={{...styles,bodyText:styles.body,helperText:styles.meta}} onOpenNotification={data=>window.opened=data}/>:<><MatchList filter="all" matches={[match]} onOpen={()=>{}} styles={styles}/>{status==='scheduled'?<><ReportPanel match={match} styles={styles}/><ShootoutPanel match={match} styles={styles}/></>:null}<FixtureHero match={{id:'test',teamName:'Our team',opponent:'Visitors',status,timerStatus:status==='scheduled'?'not_started':status==='live'?'running':status==='half_time'?'half_time':'full_time',homeScore:0,awayScore:0,matchDate:'2026-09-14',matchDurationMinutes:60,halfDurationMinutes:30}} styles={styles}/></>}</View>};createRoot(document.getElementById('root')).render(<App/>);`
...
try{
  ...
  for(const mode of ['light','dark']){
    ...
    await page.getByText('Our team v Visitors',{exact:true}).first().waitFor()
    assert.equal(await page.getByText(/^0 - 0/).count(),0)
