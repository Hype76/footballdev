...
test('single-player invitation endpoint resolves recipients server-side and uses durable idempotency', async () => {
  ...
  assert.match(invitationFunction, /event_player_change_commands/)
  assert.match(invitationFunction, /event_player_notification_events/)
  assert.match(invitationFunction, /failedLegacyRecipientEmails\.has/)
