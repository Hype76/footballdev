...
test('Hydration stays direct and shows visible paused or resume state', async () => {
  ...
  assert.match(gameModeSlice, /data-match-day-timer-action="pause"/)
  assert.match(gameModeSlice, /onTimerAction\(match, 'pause'\)/)
  assert.match(gameModeSlice, /data-match-day-timer-action="hydration"/)
  assert.match(gameModeSlice, /onTimerAction\(match, 'hydration'\)/)
  assert.match(gameModeSlice, /Match clock paused\. Use Resume to continue from the frozen time\./)
  assert.match(gameModeSlice, /data-match-day-timer-action="resume"/)
  assert.match(gameModeSlice, /onTimerAction\(match, 'resume'\)/)
