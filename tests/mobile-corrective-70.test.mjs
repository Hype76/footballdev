...
test('corrective release identity advances both native apps and authorises only guarded production candidates', async () => {
  ...
  assert.match(coachConfig, /version: '1\.0\.2[01]'/)
  assert.match(JSON.parse(coachPackage).version, /^1\.0\.2[01]$/)
  assert.match(parentConfig, /version: '1\.0\.1[78]'/)
  assert.match(JSON.parse(parentPackage).version, /^1\.0\.1[78]$/)
