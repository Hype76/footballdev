...
test('corrective release identity advances both native apps and authorises only guarded production candidates', async () => {
  ...
  assert.match(coachConfig, /version: '1\.0\.2[0-9]'/)
  assert.ok(Number(JSON.parse(coachPackage).version.split('.')[2]) >= 20)
  assert.match(parentConfig, /version: '1\.0\.(?:1[7-9]|2[0-9])'/)
  assert.ok(Number(JSON.parse(parentPackage).version.split('.')[2]) >= 17)
