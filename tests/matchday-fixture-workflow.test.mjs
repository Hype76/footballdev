...
test('staff live controls expose pause, resume, timer, and action feedback', () => {
  ...
  assert.match(source, /function formatLiveMatchClock\(match, now = Date\.now\(\), gameModePauseState = \{\}\)/)
  assert.match(source, /gameModePauseState\?\.hydrationPaused \|\| PAUSED_MATCH_STATUSES\.has\(match\.status\)/)
  assert.match(source, /const seconds = String\(elapsedSeconds % 60\)\.padStart\(2, '0'\)/)
  assert.match(source, /return `\$\{minutes\}:\$\{seconds\}`/)
  assert.match(source, /const LIVE_MATCH_CLOCK_INTERVAL_MS = 1000/)
  ...
  assert.match(source, /onHydrationToggle\(match\)/)
