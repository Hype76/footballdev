...
test('match day separates previous games without removing staff controls', () => {
  ...
  assert.match(source, /selectedMatch \? renderSelectedMatchWorkspace\(selectedMatch\) : null/)
...
test('phone and desktop Game Day render list-first navigation and one selected workspace', () => {
  ...
  assert.match(source, /selectedMatch \? 'hidden xl:block' : ''/)
  assert.match(source, />\s*Back to fixtures\s*</)
  assert.match(source, /xl:grid xl:grid-cols-\[20rem_minmax\(0,1fr\)\]/)
  assert.match(source, /xl:max-h-\[calc\(100vh-9rem\)\] xl:overflow-y-auto/)
  assert.match(source, /selectedMatch \? renderSelectedMatchWorkspace\(selectedMatch\) : null/)
