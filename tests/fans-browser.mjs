...
const server = await createServer({ cacheDir: 'node_modules/.vite-fan-feedback', optimizeDeps: { entries: ['index.html'] }, server: { host: '127.0.0.1', port, strictPort: true }, mode: 'development' })
await server.listen()
...
try {
  ...
  await page.goto(`${origin}/fans`, { timeout: 120000 })
  await page.getByLabel('Player', {exact:true}).selectOption('parent-link-fixture-second')
  ...
  assert.equal(await page.getByLabel('Player',{exact:true}).inputValue(), 'parent-link-fixture-second')
  ...
  connections.push({id:'30000000-0000-4000-8000-000000000001',is_owner:false,status:'active',relationship_type:'fan',player_name:'Followed child',club_name:'Test club',team_name:'Test team',plan_key:'club',plan_status:'active',permissions:{schedule:true,game_day:false,development:false,resources:false}})
  await page.reload()
  ...
  await page.getByRole('button',{name:'Remove my access to Followed child',exact:true}).click()
  await page.getByRole('dialog').getByRole('button',{name:'Remove access',exact:true}).click()
  ...
  await page.getByRole('navigation',{name:'Fan navigation'}).getByRole('button',{name:'Fans',exact:true}).click()
  ...
  await page.getByRole('button',{name:'Create account',exact:true}).click()
  await page.getByRole('alert').getByText('Password must be at least 8 characters.',{exact:true}).waitFor()
  assert.equal(signupRequests,0)
  await page.getByLabel('Password',{exact:true}).fill('Test-River-729!')
  await page.getByLabel('Password',{exact:true}).press('Enter')
  await page.getByRole('alert').getByText(/Account creation could not be completed/).waitFor()
  ...
  assert.match(await page.getByRole('alert').innerText(), /If you are new, choose Create account first/)
