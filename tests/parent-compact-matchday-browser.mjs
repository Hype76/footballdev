...
try {
  ...
  await page.getByRole('button', { name: 'Hide formation', exact: true }).click()
  assert.equal(await page.getByText('Match plans', { exact: true }).count(), 0)
  await page.evaluate(() => window.match({ formationPlan: null, formationPlans: [] }))
  await page.getByRole('button', { name: 'Show formation', exact: true }).waitFor({ state: 'detached' })
  assert.equal(await page.getByRole('button', { name: 'Show formation', exact: true }).count(), 0)
  await page.evaluate(() => window.match({}))
  await page.getByRole('button', { name: 'Show formation', exact: true }).waitFor()
