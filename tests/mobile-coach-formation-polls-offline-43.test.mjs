...
test('Coach app exposes simplified Formation save and Parent publication through canonical RPCs', async () => {
  ...
  for (const copy of ['Choose formation', 'Select full squad', 'Load empty pitch', 'Build lineup', 'Take Players off', 'Move ${removalIds.length || \'\'} selected to Bench', 'Move to pitch', 'Continue to save', 'Save Formation Board', 'Save and link to match', 'Save and publish to Team Resources']) assert.match(screen, new RegExp(copy.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')))
