...
test('app wiring uses the timer RPC and removes local Game Mode pause state', async () => {
  ...
  assert.match(page, /const handleGameModeTimerAction = async \(match, action\) =>/)
  assert.match(page, /onTimerAction\(match, 'pause'\)/)
  assert.match(page, /onTimerAction\(match, 'hydration'\)/)
  assert.match(page, /onTimerAction\(match, 'resume'\)/)
