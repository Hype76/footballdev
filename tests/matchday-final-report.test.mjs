...
test('completed game and Previous Games UI expose shared mobile-safe staff and parent event reports', async () => {
  ...
  const previousStart = page.indexOf('{isPreviousGamesOpen ? (')
  const previousEnd = page.indexOf('</section>', previousStart)
  ...
  assert.match(completedEvents, /setOpenSectionId\(\(current\) => current === sectionId \? '' : sectionId\)/)
  assert.doesNotMatch(completedEvents, /overflow-y-auto|max-h-\[32rem\]/)
