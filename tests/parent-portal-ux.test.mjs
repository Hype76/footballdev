...
test('parent child selector is placed before dashboard content', async () => {
  ...
  const navIndex = source.indexOf('<ParentPortalRouteShell')
...
test('parent dashboard exposes surfaced parent links without feature clutter', async () => {
  ...
  assert.match(shellSource, /variant === 'mobile' \? null/)
  assert.match(shellSource, /lg:grid-cols-\[16rem_minmax\(0,1fr\)\]/)
  assert.match(shellSource, /hidden lg:flex/)
  assert.match(shellSource, /pb-\[max\(0\.5rem,env\(safe-area-inset-bottom\)\)\]/)
  assert.match(shellSource, /h-full min-h-0 flex-col overflow-hidden/)
...
test('parent scorer actions use app modal fields instead of browser dialogs', async () => {
  ...
  assert.doesNotMatch(source, /openParentMatchActionModal\(\{ type: 'voidGoal'/)
  ...
  assert.doesNotMatch(modalSource, /Removal reason/)
  assert.doesNotMatch(source, /voidMatchDayGoalAsScorer|onVoidGoal|handleVoidGoal|Undo event/)
