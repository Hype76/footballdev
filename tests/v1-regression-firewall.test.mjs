...
test('Development Forms remains visible and Development Fields direct route stays preserved', async () => {
  ...
  assert.match(navigation, /label: 'Development Forms'[\s\S]*path: '\/feedback-forms'/)
  ...
  assert.match(roleQuickLinks, /label: 'Development Forms', path: '\/feedback-forms'/)
...
test('calendar and Match Day approved actions remain present', async () => {
  ...
  assert.match(calendar, /if \(isMatchday && requestedEventType && requestedEventType !== 'match'\)/)
  assert.match(calendar, /const eventType = isMatchday \? 'match' : \(isClubWideCalendar \|\| calendarOnly\) \? 'general' : defaultForm\.eventType/)
  assert.match(calendar, /if \(requestedEventType === 'match'\) \{[\s\S]*openCalendarMatchDayWorkflow\(/)
