...
test('role access matrix grants only approved staff roles', () => {
  ...
  assert.equal(canUseDataTransfer({ ...base, role: 'coach', roleRank: 30 }), true)
  assert.equal(canUseDataTransfer({ ...base, role: 'assistant_coach', roleRank: 20 }), false)
  assert.equal(canUseDataTransfer({ ...base, role: 'coach', roleRank: 30, planKey: 'single_team' }), false)
...
test('server flow exposes inspect and preview before the separate confirmation RPC', async () => {
  ...
  assert.match(source, /resolveDataTransferTeamSelection/)
  assert.match(source, /CONFIRMATION_PLAN_HASH_MISMATCH/)
  ...
  assert.match(source, /getDataTransferFieldPolicy\(actor\)/)
  assert.match(source, /applyDataTransferExportFieldPolicy/)
  ...
  assert.match(source, /requiresSingleTeamSelection/)
