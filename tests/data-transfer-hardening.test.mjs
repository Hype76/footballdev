...
test('server flow exposes inspect and preview before the separate confirmation RPC', async () => {
  ...
  assert.match(source, /operation === 'source-inspect'/)
  assert.match(source, /operation === 'simple-template'/)
  assert.match(source, /operation === 'ordinary-export'/)
  ...
  assert.match(source, /TABULAR_FORMATS\[source\.format\]/)
