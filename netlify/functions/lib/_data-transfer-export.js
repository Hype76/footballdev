...
async function buildOds(definition, rows) {
  const zip = new JSZip()
  zip.file('mimetype', ORDINARY_EXPORT_FORMATS.ods.mimeType, { compression: 'STORE' })
...
<manifest:file-entry manifest:full-path="/" manifest:media-type="${ORDINARY_EXPORT_FORMATS.ods.mimeType}"/>
