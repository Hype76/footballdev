...
import {
  ...
  DATA_TRANSFER_MAX_COLUMNS,
  DATA_TRANSFER_MAX_ROWS,
...
export const TABULAR_MAX_BYTES = DATA_TRANSFER_MAX_BYTES
export const TABULAR_MAX_ROWS = DATA_TRANSFER_MAX_ROWS
export const TABULAR_MAX_COLUMNS = DATA_TRANSFER_MAX_COLUMNS
export const TABULAR_FORMATS = DATA_TRANSFER_FORMATS
...
async function detectZipFormat(buffer) {
  ...
  if (odsMime === TABULAR_FORMATS.ods.storageMimeType) return 'ods'
...
export async function detectSpreadsheetFormat(buffer, { fileName = '', mimeType = '' } = {}) {
  ...
  const suppliedMime = normalizeDataTransferMimeType(mimeType)
  const allowedMimes = TABULAR_FORMATS[detected].recognizedMimeAliases
...
async function inspectOds(buffer) {
  ...
  if (mimeType !== TABULAR_FORMATS.ods.storageMimeType) throw dataError('The file is not an OpenDocument Spreadsheet.', 'INVALID_ODS_MIME')
...
function storedTransferReference(entity) {
  return normalizeText(entity?.transfer_reference)
}

function planningHandle(entityType, values) {
  return stableReference(`PLAN-${entityType}`, values)
}
...
export async function mapSpreadsheetToTransferRows(buffer, {
  ...
  const existingTeamsByNormalizedName = new Map()
  for (const team of existingTeams) {
    const key = normalizeHeader(team.name)
    if (!existingTeamsByNormalizedName.has(key)) existingTeamsByNormalizedName.set(key, [])
    existingTeamsByNormalizedName.get(key).push(team)
  }
  ...
  function resolveTeam(value, rowNumber) {
    ...
    const exactMatches = existingTeams.filter((team) => (
      authorizedTeamIds.has(team.id)
      && normalizeText(team.name) === sourceTeamName
    ))
    if (exactMatches.length === 1) return exactMatches[0]
    if (exactMatches.length > 1) {
      errors.push({ sheet: dataSheet.name, row: rowNumber, column: 'Team', code: 'TEAM_MATCH_AMBIGUOUS', message: `More than one authorised team is named ${sourceTeamName}. Choose an explicit team mapping.` })
      return null
    }
    const normalizedMatches = (existingTeamsByNormalizedName.get(normalizeHeader(sourceTeamName)) || [])
      .filter((team) => authorizedTeamIds.has(team.id))
    if (normalizedMatches.length === 1) return normalizedMatches[0]
    if (normalizedMatches.length > 1) {
      errors.push({ sheet: dataSheet.name, row: rowNumber, column: 'Team', code: 'TEAM_MATCH_AMBIGUOUS', message: `More than one authorised team matches ${sourceTeamName}. Choose an explicit team mapping.` })
      return null
    }
    if (decision?.create) {
  ...
  function addGuardian(values, playerPlanningHandle, playerTransferReference, rowNumber, index) {
    ...
    if (!guardian) {
      const transferReference = existingGuardian
        ? storedTransferReference(existingGuardian)
        : stableReference('GUARDIAN', [scope.clubId, firstName, lastName, email || phone || `${rowNumber}:${index}`])
      guardian = {
        _planningHandle: planningHandle('GUARDIAN', [scope.clubId, existingGuardian?.id || shareKey]),
        _resolvedEntityId: existingGuardian?.id || '',
    ...
    linkRows.push({
      _guardianPlanningHandle: guardian._planningHandle,
      _playerPlanningHandle: playerPlanningHandle,
      _sourceRow: rowNumber,
      player_reference: playerTransferReference,
  ...
  dataSheet.rows.forEach((sourceRow, rowIndex) => {
    ...
    if (!team) return
    const teamPlanningHandle = planningHandle('TEAM', [scope.clubId, team.id || `new:${team.name}`])
    const teamTransferReference = team._create ? normalizeText(team.transfer_reference) : storedTransferReference(team)
    usedTeams.set(teamPlanningHandle, {
      _planningHandle: teamPlanningHandle,
      _resolvedEntityId: team.id || '',
      _sourceRow: rowNumber,
      transfer_reference: teamTransferReference,
      name: team.name,
      season: team._create ? importOptions.season : normalizeText(team.season),
    ...
    const playerPlanningHandle = planningHandle('PLAYER', [scope.clubId, existingPlayer?.id || identity, teamPlanningHandle])
    const playerTransferReference = existingPlayer
      ? storedTransferReference(existingPlayer)
      : stableReference('PLAYER', [scope.clubId, firstName, lastName, values.date_of_birth, teamPlanningHandle])
    const duplicateSourceRow = seenPlayerKeys.get(playerPlanningHandle)
    ...
    seenPlayerKeys.set(playerPlanningHandle, rowNumber)
    playerRows.push({
      _planningHandle: playerPlanningHandle,
      _resolvedEntityId: existingPlayer?.id || '',
      _teamPlanningHandle: teamPlanningHandle,
      _sourceRow: rowNumber,
      transfer_reference: playerTransferReference,
      team_reference: teamTransferReference,
      first_name: firstName,
      last_name: lastName,
    ...
    addGuardian(values, playerPlanningHandle, playerTransferReference, rowNumber, 1)
    addGuardian(values, playerPlanningHandle, playerTransferReference, rowNumber, 2)
  })
  ...
  return {
    ...
    rowsBySheet: {
      'Club Details': [{
        _planningHandle: planningHandle('CLUB', [scope.clubId]),
        _resolvedEntityId: existing.club.id,
        _sourceRow: 2,
        transfer_reference: storedTransferReference(existing.club),
        name: existing.club.name,
        season: normalizeText(existing.club.season),
      }],
    ...
    sourceMetadata: {
      ...
      selectedSeason: normalizeText(importOptions.season),
...
async function buildSimpleOds(scopeLabel) {
  const zip = new JSZip()
  zip.file('mimetype', TABULAR_FORMATS.ods.storageMimeType, { compression: 'STORE' })
...
<manifest:file-entry manifest:full-path="/" manifest:media-type="${TABULAR_FORMATS.ods.storageMimeType}"/>
...
export async function buildSimpleTransferTemplate(format, { scopeLabel = '' } = {}) {
  ...
  if (format === 'xlsx') {
    return { buffer: await buildSimpleXlsx(scopeLabel), filename: `${base}.xlsx`, mimeType: TABULAR_FORMATS.xlsx.responseMimeType }
  }
  if (format === 'ods') {
    return { buffer: await buildSimpleOds(scopeLabel), filename: `${base}.ods`, mimeType: TABULAR_FORMATS.ods.responseMimeType }
  }
  const csv = `\uFEFF${simpleTemplateRows().map((row) => row.map((value) => quoteDelimited(value)).join(',')).join('\r\n')}\r\n`
  return { buffer: Buffer.from(csv, 'utf8'), filename: `${base}.csv`, mimeType: TABULAR_FORMATS.csv.responseMimeType }
}
