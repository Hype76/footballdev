...
export async function deliverSquadDecisionNotifications(ids, { admin = supabaseAdmin, deliver = deliverMatchDayNotification, email = sendEmail } = {}) {
  let completed = 0
  for (const id of ids) {
    ...
    try {
      ...
      if (receipt.delivery_channel === 'email') {
        ...
        await email({
          ...
          subject: `${clubName}: Squad update`, text: `${receipt.body}\n\n${clubName}`,
        }, {
