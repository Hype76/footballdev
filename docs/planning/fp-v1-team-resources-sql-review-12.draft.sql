DO NOT RUN
Draft only for FP-V1-TEAM-RESOURCES-SQL-REVIEW-12
Requires explicit Steve approval before execution

/*
This is a planning artifact, not an active migration.

Target production ref: hvapkizujvsahvgspser
Source schema dump: C:/Users/pulse/AppData/Local/Temp/fp-prod-schema-public.sql

Purpose:
- Add explicit parent sharing for player-linked team resources.
- Add an additive external-link storage model without changing existing file
  constraints on public.resource_library_items.
- Expose parent-visible resources through an RPC scoped by public.parent_player_links.

Stop before execution if any object shape differs from the reviewed dump.
*/
...
drop policy if exists resource_library_external_links_select_scoped
...
to authenticated
using (
  public.current_user_can_view_resource_library(club_id, team_id)
  or exists (
    select 1
    from public.resource_library_links link
    join public.parent_player_links parent_link
      on parent_link.club_id = link.club_id
     and parent_link.player_id = link.linked_id
     and parent_link.auth_user_id = auth.uid()
     and parent_link.status = 'active'
    where link.resource_id = resource_library_external_links.resource_id
      and link.club_id = resource_library_external_links.club_id
      and link.team_id = resource_library_external_links.team_id
...
drop policy if exists resource_library_items_select_parent_visible
on public.resource_library_items;
create policy resource_library_items_select_parent_visible
on public.resource_library_items
for select
to authenticated
using (
  ...
  and exists (
    ...
    join public.parent_player_links parent_link
     ...
     and parent_link.auth_user_id = auth.uid()
     and parent_link.status = 'active'
    where link.resource_id = resource_library_items.id
      and link.club_id = resource_library_items.club_id
      and link.team_id = resource_library_items.team_id
      and link.linked_type = 'player'
      and link.parent_visible is true
      and link.removed_at is null
  )
...
drop policy if exists resource_library_links_select_parent_visible
on public.resource_library_links;
create policy resource_library_links_select_parent_visible
on public.resource_library_links
for select
to authenticated
using (
  linked_type = 'player'
  and parent_visible is true
  and removed_at is null
  and exists (
    select 1
    from public.parent_player_links parent_link
    where parent_link.club_id = resource_library_links.club_id
      and parent_link.player_id = resource_library_links.linked_id
      and parent_link.auth_user_id = auth.uid()
      and parent_link.status = 'active'
  )
);
