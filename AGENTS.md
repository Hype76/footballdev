...
Requested product fixes and changes include production deployment by default unless the user says otherwise. Complete required checks, deploy the authorized changes, verify the live result, and report when live without asking for deployment approval again.

When emitting Codex app directives on Windows, use forward-slash absolute paths like `E:/Project Manager/Footbal_Development`. Do not use raw backslash paths in directive attributes.
