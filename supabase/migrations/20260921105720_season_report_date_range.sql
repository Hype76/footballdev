-- Explicit inclusive dates for season reporting. Keep the legacy RPC unchanged.
...
REVOKE ALL ON FUNCTION app_private.end_season_stats_range(uuid, date, date) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION app_private.end_season_stats_range(uuid, date, date) TO authenticated;

CREATE OR REPLACE FUNCTION public.get_end_season_stats_range(team_id_value uuid, start_date_value date, end_date_value date)
RETURNS TABLE(player_id uuid, player_name text, shirt_number text, team_id uuid, team_name text, goals integer, assists integer, motm_votes integer)
LANGUAGE sql STABLE SECURITY INVOKER SET search_path = ''
AS $function$
  SELECT * FROM app_private.end_season_stats_range(team_id_value, start_date_value, end_date_value);
$function$;
REVOKE ALL ON FUNCTION public.get_end_season_stats_range(uuid, date, date) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_end_season_stats_range(uuid, date, date) TO authenticated;
