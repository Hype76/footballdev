-- Event participation belongs to the saved event, even after a Player leaves its roster.
-- Retain club and staff authority, completed-event protection, and existing history.
...
declare
  ...
  source_team_id uuid;
  source_title text;
  match_in_progress boolean := false;
...
begin
    ...
    where player.id = player_id_value
      ...
      and (
        exists (
          ...
          where invite.club_id = source_club_id and invite.team_id = source_team_id
            and invite.player_id = player_id_value
            and ((normalized_source_type = 'calendar' and invite.calendar_event_id = event_id_value)
              or (normalized_source_type = 'match-day' and invite.match_day_id = event_id_value))
        )
        or (normalized_source_type = 'calendar' and exists (
          ...
          where request.calendar_event_id = event_id_value
            and recipient.club_id = source_club_id and recipient.team_id = source_team_id
            and recipient.player_id = player_id_value
        ))
        or (normalized_source_type = 'match-day' and exists (
          select 1 from public.match_day_availability_requests request
          where request.match_day_id = event_id_value
            and request.club_id = source_club_id and request.team_id = source_team_id
        ...
        or (normalized_source_type = 'match-day' and exists (
          ...
          where decision.match_day_id = event_id_value
            and decision.club_id = source_club_id and decision.team_id = source_team_id
            and decision.player_id = player_id_value
        ))
      )
  ) then
    raise exception 'The Player is not attached to this event in the active club.';
  end if;
  ...
  select (
    ...
    or (normalized_source_type = 'match-day' and (
      exists (
        select 1 from public.match_day_availability_requests request
        where request.match_day_id = event_id_value
          and request.club_id = source_club_id and request.team_id = source_team_id
          and request.player_id = player_id_value and request.token_revoked_at is null
      )
      or exists (
        select 1 from public.match_day_player_squad_decisions decision
        where decision.match_day_id = event_id_value
          and decision.club_id = source_club_id and decision.team_id = source_team_id
          and decision.player_id = player_id_value and decision.status = 'selected'
      )
    ))
  ...
  if normalized_source_type = 'match-day' or recurrence_frequency_value = 'none' then
    ...
    if not match_in_progress and source_end <= timezone('utc', now()) then
      raise exception 'Completed event participation cannot be removed from history.';
    end if;

    requires_in_progress_confirmation := match_in_progress or source_start <= timezone('utc', now());
...
-- Replace the temporary team-reassignment wrapper with direct participation removal.
...
begin
  ...
  previous_state_value := jsonb_build_object(
    ...
    'teamMembership', case when exists (
      select 1 from public.players player
      where player.id = player_id_value and player.club_id = source_club_id
        and player.team_id = source_team_id and coalesce(player.status, 'active') <> 'archived'
    ) then 'active' else 'not_current' end,
  ...
  insert into public.event_player_removal_commands (
    ...
    jsonb_build_object(
      ...
      'scope', normalized_scope,
      'teamMembership', previous_state_value ->> 'teamMembership',
