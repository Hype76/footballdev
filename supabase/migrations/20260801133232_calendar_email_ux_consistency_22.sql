...
declare
  ...
  normalized_enabled boolean := coalesce(enabled_value, false);
  normalized_notify boolean := case
    when coalesce(enabled_value, false) then true
    else coalesce(notify_invited_families_value, false)
  end;
begin
  ...
  if normalized_enabled then
    ...
    where id = event_record.id
      and club_id = event_record.club_id
      and team_id = event_record.team_id;
  end if;
...
comment on function public.save_training_availability_setting_v3(uuid, boolean, integer, boolean) is
  'Saves canonical Training RSVP settings and enforces involved-player family visibility plus required RSVP notification state when enabled.';
