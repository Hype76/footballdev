...
add column if not exists allow_vote_changes boolean not null default true;
...
begin
  ...
  if existing_vote_id is not null then
    if poll_row.allow_vote_changes is false then
      raise exception 'Your vote is locked for this poll.';
    end if;
