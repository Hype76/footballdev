...
alter table public.match_days
  add column if not exists request_scorer boolean not null default false,
  add column if not exists request_linesman boolean not null default false,
  add column if not exists request_referee boolean not null default false;

update public.match_days
set request_scorer = true
where status = 'scorer_request'
  or trim(coalesce(scorer_request_message, '')) <> '';
