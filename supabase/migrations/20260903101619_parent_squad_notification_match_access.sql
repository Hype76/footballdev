-- Squad notification destinations must use the same current parent scope as the match list.
...
as $$
  ...
  select
    ...
    availability.status, availability.selected_at, coalesce(decision.status, 'undecided'), decision.updated_at,
  ...
  left join public.match_day_player_availability availability
    on availability.match_day_id = fixture.id
    and availability.club_id = fixture.club_id
    and availability.team_id is not distinct from fixture.team_id
    and availability.player_id = link.player_id
  where fixture.deleted_at is null
    ...
    and (
      ...
      or (
        (decision.status = 'selected' or decision.notified_at is not null)
        and exists (
          select 1 from public.players player
          where player.id = link.player_id and player.club_id = fixture.club_id
            and player.team_id = fixture.team_id and coalesce(player.status, 'active') <> 'archived'
        )
        and exists (select 1 from public.clubs club where club.id = fixture.club_id and coalesce(club.status, 'active') = 'active')
...
comment on function public.get_parent_portal_match_days(uuid) is
  'Returns authorised fixtures for invitations, selected players, and current notified squad decisions. Selection grants match access without changing attendance or creating invitations.';
...
as $$
  ...
  available_selected_players as materialized (
    select distinct
    ...
    join public.match_day_player_availability availability
      on availability.match_day_id = fixture.match_day_id
      and availability.club_id = fixture.club_id
      and availability.team_id is not distinct from fixture.team_id
      and availability.player_id = decision.player_id
  ...
  from visible_fixtures fixture
  left join available_selected_players selected
...
comment on function public.get_parent_portal_confirmed_teams(uuid) is
  'Returns only ordered display names for currently Available and Selected players in parent-visible fixtures for the authenticated parent link team.';
