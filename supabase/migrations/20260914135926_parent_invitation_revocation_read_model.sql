-- Keep response eligibility aligned with the canonical token check; never reactivate removed participation.
...
as $$
  ...
  normalized as (
    select
      ...
      case when legacy.invitation_type = 'match_attendance' then coalesce(match_request.id, legacy.source_record_id) else legacy.source_record_id end as source_record_id,
      legacy.source_type,
      ...
      legacy.role_type,
      case
        when legacy.invitation_type in ('match_attendance', 'match_role') and match_request.token_revoked_at is not null then 'cancelled'
        when legacy.invitation_type = 'match_attendance'
          and public.is_match_day_action_token_current_internal(match_request.token_hash)
          and legacy.event_date >= timezone('Europe/London', now())::date
          and match_day.concluded_at is null then 'active'
      ...
      case
        when legacy.invitation_type in ('match_attendance', 'match_role')
          and (match_request.id is null or not public.is_match_day_action_token_current_internal(match_request.token_hash)) then false
      ...
      case
        when legacy.invitation_type in ('match_attendance', 'match_role') and match_request.token_revoked_at is not null
          then 'This invitation was withdrawn. Ask the Coach for a new invitation if needed.'
        when legacy.invitation_type in ('match_attendance', 'match_role')
          and (match_request.id is null or not public.is_match_day_action_token_current_internal(match_request.token_hash))
          then 'This invitation is no longer active.'
      ...
      case when legacy.invitation_type = 'match_attendance' then coalesce(match_request.expires_at, legacy.response_deadline) else legacy.response_deadline end as response_deadline,
      case
    ...
    left join lateral (
      ...
      where legacy.invitation_type in ('match_attendance', 'match_role')
        and (legacy.invitation_type = 'match_attendance' or request.id = legacy.source_record_id)
      ...
      order by
        case when public.is_match_day_action_token_current_internal(request.token_hash) then 0 else 1 end,
...
comment on function public.get_parent_portal_invitation_state(uuid) is
  'Returns current actionable Match Day requests and preserves withdrawn invitation history. One attendance request per recurring occurrence. Attendance is shared by active contacts for the child while volunteer offers remain contact-specific.';
