-- FP-V1-CALENDAR-RESPONSE-POLISH-10A
    ...
    where request.token_hash = lower(btrim(coalesce(token_hash_value, '')))
      ...
      and (
        ...
        or (
          ...
          and (
            lower(btrim(coalesce(player.parent_email, ''))) = lower(btrim(request.recipient_email))
            or exists (
              select 1
...
comment on function public.is_match_day_action_token_current_internal(text) is
  'Validates a current Match Day response token against fixture, player, tenant, expiry, active linked-parent authority, adult-player authority, or a still-current server-side parent contact. Stale contacts remain denied.';
...
begin
  ...
  if normalized_event_type = 'match' then
    ...
    if not exists (
      ...
      raise exception 'This player is not actively invited to the fixture.';
    end if;
  ...
  if normalized_event_type = 'training' then
    ...
    if not exists (
      select 1
      from public.calendar_event_invites invite
      where invite.calendar_event_id = calendar_event_row.id
        and invite.club_id = calendar_event_row.club_id
        and invite.team_id = calendar_event_row.team_id
        and invite.player_id = player_row.id
        and invite.invite_status <> 'cancelled'
        and invite.cancelled_at is null
    ) then
      raise exception 'This player is not actively invited to the training event.';
    end if;
...
create unique index if not exists scheduled_email_queue_single_player_invitation_action_key
...
comment on index public.scheduled_email_queue_single_player_invitation_action_key is
  'Prevents a deliberate single-player invitation action from queuing twice with the same server-validated idempotency key.';

create table if not exists public.event_player_invitation_actions (
  ...
  action text not null check (action in ('send', 'resend', 'retry')),
...
create index if not exists event_player_invitation_actions_event_player_idx
on public.event_player_invitation_actions (club_id, team_id, source_type, event_id, player_id, created_at desc);

alter table public.event_player_invitation_actions enable row level security;
alter table public.event_player_invitation_actions force row level security;

revoke all on public.event_player_invitation_actions from public, anon, authenticated;
grant select, insert, update on public.event_player_invitation_actions to service_role;

comment on table public.event_player_invitation_actions is
  'Server-only idempotency and audit state for deliberate one-player invitation sends, resends, and retries.';
