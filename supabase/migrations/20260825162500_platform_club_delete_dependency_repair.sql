begin;

alter table public.club_owner_invites
  drop constraint if exists club_owner_invites_team_id_fkey;

alter table public.club_owner_invites
  add constraint club_owner_invites_team_id_fkey
  foreign key (team_id)
  references public.teams (id)
  on delete cascade;

create or replace function public.delete_platform_club_cascade(target_club_id uuid)
returns void
language plpgsql
security definer
set search_path = public
as $$
declare
  target_club_name text;
begin
  if auth.uid() is null then
    raise exception 'Login is required before deleting a club workspace.';
  end if;

  if not exists (
    select 1
    from public.users
    where id = auth.uid()
      and role = 'super_admin'
  ) then
    raise exception 'Only platform admins can delete clubs.';
  end if;

  if target_club_id is null then
    raise exception 'Club ID is required.';
  end if;

  select name
  into target_club_name
  from public.clubs
  where id = target_club_id;

  delete from public.user_club_memberships membership
  where membership.club_id = target_club_id
     or not exists (
       select 1
       from public.clubs club
       where club.id = membership.club_id
     );

  if target_club_name is null then
    return;
  end if;

  delete from public.workspace_team_transfer_requests request
  where request.source_club_id = target_club_id
     or request.destination_club_id = target_club_id
     or exists (
       select 1
       from public.teams team
       where team.id = request.team_id
         and team.club_id = target_club_id
     );

  delete from public.platform_access_assignment_history history
  where history.club_id = target_club_id
     or exists (
       select 1
       from public.teams team
       where team.id = history.team_id
         and team.club_id = target_club_id
     );

  delete from public.player_team_removal_commands command
  where command.club_id = target_club_id
     or exists (
       select 1
       from public.teams team
       where team.id = command.team_id
         and team.club_id = target_club_id
     );

  delete from public.adult_player_account_links account_link
  where account_link.club_id = target_club_id
     or exists (
       select 1
       from public.teams team
       where team.id = account_link.team_id
         and team.club_id = target_club_id
     );

  delete from public.data_transfer_batches
  where club_id = target_club_id;

  delete from public.club_owner_invites invite
  where invite.club_id = target_club_id
     or exists (
       select 1
       from public.teams team
       where team.id = invite.team_id
         and team.club_id = target_club_id
     );

  delete from public.parent_portal_message_reads read_row
  where exists (
    select 1
    from public.parent_player_links link
    where link.id = read_row.parent_link_id
      and link.club_id = target_club_id
  )
  or exists (
    select 1
    from public.communication_logs log
    where log.id = read_row.communication_log_id
      and log.club_id = target_club_id
  );

  delete from public.parent_push_subscriptions
  where club_id = target_club_id;

  delete from public.match_day_scorer_assignments assignment
  where exists (
    select 1
    from public.match_days match_day
    where match_day.id = assignment.match_day_id
      and match_day.club_id = target_club_id
  );

  delete from public.match_day_scorer_interest interest
  where exists (
    select 1
    from public.match_days match_day
    where match_day.id = interest.match_day_id
      and match_day.club_id = target_club_id
  );

  delete from public.match_day_events
  where club_id = target_club_id;

  delete from public.match_days
  where club_id = target_club_id;

  delete from public.match_locations
  where club_id = target_club_id;

  delete from public.polls
  where club_id = target_club_id;

  delete from public.parent_player_links
  where club_id = target_club_id;

  delete from public.parent_email_templates
  where club_id = target_club_id;

  delete from public.player_staff_notes
  where club_id = target_club_id;

  delete from public.assessment_session_games
  where club_id = target_club_id;

  delete from public.assessment_sessions
  where club_id = target_club_id;

  delete from public.record_backups
  where club_id = target_club_id;

  delete from public.communication_logs
  where club_id = target_club_id;

  delete from public.evaluations
  where club_id = target_club_id;

  delete from public.form_fields
  where club_id = target_club_id;

  delete from public.club_roles
  where club_id = target_club_id;

  delete from public.club_user_invites
  where club_id = target_club_id;

  delete from public.team_staff staff
  where exists (
    select 1
    from public.teams team
    where team.id = staff.team_id
      and team.club_id = target_club_id
  );

  delete from public.teams
  where club_id = target_club_id;

  delete from public.players
  where club_id = target_club_id;

  delete from public.platform_feedback
  where club_id = target_club_id;

  update public.stripe_checkout_records
  set club_id = null,
      updated_at = now()
  where club_id = target_club_id;

  update public.users
  set club_id = null
  where club_id = target_club_id
    and role = 'super_admin';

  delete from public.users
  where club_id = target_club_id
    and role <> 'super_admin';

  delete from public.clubs
  where id = target_club_id;
end;
$$;

revoke all on function public.delete_platform_club_cascade(uuid)
from public, anon;
grant execute on function public.delete_platform_club_cascade(uuid)
to authenticated, service_role;

comment on function public.delete_platform_club_cascade(uuid) is
  'Permanently deletes an archived Club after removing all restricted Team and Club dependencies in a safe order.';

commit;
