...
add column if not exists allow_own_child_votes boolean not null default true;
...
begin
  ...
  if poll_row.allow_own_child_votes is false
    and nullif(selected_option ->> 'playerId', '') is not null
    and nullif(selected_option ->> 'playerId', '')::uuid = link_row.player_id
  then
