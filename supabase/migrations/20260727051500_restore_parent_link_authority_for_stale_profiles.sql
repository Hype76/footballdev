...
-- Keep active Parent-link authority independent from stale staff profile state.
-- An explicitly suspended account remains blocked.
...
comment on function public.current_user_can_access_parent_player(uuid) is
  'Allows an active account to read only active, non-archived players linked directly to that account. Stale staff profiles do not cancel Parent-link authority.';
comment on function public.current_user_can_access_parent_team(uuid) is
  'Allows an active account to read only teams reached through its active, non-archived Parent links. Stale staff profiles do not cancel Parent-link authority.';
comment on function public.current_user_can_access_parent_link(uuid, uuid) is
  'Allows an active account to read only its own active Parent links for non-archived players. Stale staff profiles do not cancel Parent-link authority.';
comment on function public.current_user_can_access_parent_club(uuid) is
  'Allows an active account to read only clubs reached through its active, non-archived Parent links. Stale staff profiles do not cancel Parent-link authority.';
