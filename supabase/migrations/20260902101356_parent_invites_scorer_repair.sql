-- Use active Parent links for scorer authority and allow staff responses for attached unsent players.
  ...
  select (select auth.uid()) is not null
    and exists (
      ...
      where match_day.id = target_match_day_id
        ...
        and public.current_user_can_access_parent_link(parent_link.id, parent_link.player_id)
        and exists (select 1 from public.clubs club where club.id = match_day.club_id and coalesce(club.status, 'active') = 'active')
    );
$$;

revoke all on function public.current_user_has_match_day_scorer_assignment(uuid) from public, anon;
grant execute on function public.current_user_has_match_day_scorer_assignment(uuid) to authenticated, service_role;
...
revoke all on function public.accept_event_player_availability_on_behalf(text, uuid, uuid, date) from public, anon;
grant execute on function public.accept_event_player_availability_on_behalf(text, uuid, uuid, date) to authenticated, service_role;

alter type public.email_delivery_state_v1 add value if not exists 'retrying_app_notification';
alter type public.email_delivery_state_v1 add value if not exists 'provider_accepted_app_notification_failed';
alter type public.email_delivery_state_v1 add value if not exists 'app_notification_failed';
alter type public.email_delivery_state_v1 add value if not exists 'app_notification_accepted';
