alter table public.match_days
  add column if not exists auto_select_available_players boolean not null default false;

alter table public.match_days
  alter column auto_select_available_players set default true;

comment on column public.match_days.auto_select_available_players is
  'When enabled, a newly Available response attempts a one-way server-authoritative squad selection. Existing fixtures were retained as disabled when this column was introduced.';
...
begin
  ...
  if match_row.id is null then
    ...
    if player_row.id is null
      ...
      failure_reason := 'The player is not an active squad player for the fixture team.';
    elsif not exists (
      select 1
...
drop trigger if exists match_day_available_auto_selection
on public.match_day_player_availability;

create trigger match_day_available_auto_selection
after insert or update of status
on public.match_day_player_availability
for each row
execute function public.handle_match_day_available_auto_selection();

comment on function public.handle_match_day_available_auto_selection() is
  'Attempts one-way selection when an invited match player becomes Available. Selection failures are contained so availability is preserved, with safe event and audit metadata and no notification side effects.';
