-- FP-V1-CALENDAR-INVITE-RECIPIENT-INTEGRITY-34

create or replace function public.canonical_calendar_invite_recipient_type(
  recipient_class_value text
)
returns text
language plpgsql
immutable
...
as $$
declare
  normalized_recipient_class text := lower(btrim(coalesce(recipient_class_value, '')));
begin
  return case normalized_recipient_class
    when 'parent' then 'parent_guardian'
    when 'guardian' then 'parent_guardian'
    when 'parent_guardian' then 'parent_guardian'
    when 'adult_player' then 'player'
    when 'player' then 'player'
    when 'parent_and_player' then 'parent_and_player'
    else null
  end;
end;
$$;

revoke all on function public.canonical_calendar_invite_recipient_type(text)
from public, anon, authenticated;
grant execute on function public.canonical_calendar_invite_recipient_type(text)
to service_role;

comment on function public.canonical_calendar_invite_recipient_type(text) is
  'Maps approved domain recipient classes to the closed Calendar invitation recipient contract. Unknown values return null and are rejected before persistence.';

create or replace function public.enforce_calendar_invite_recipient_type()
returns trigger
language plpgsql
security invoker
set search_path = ''
as $$
declare
  canonical_recipient_type text;
begin
  canonical_recipient_type := public.canonical_calendar_invite_recipient_type(new.recipient_type);

  if canonical_recipient_type is null then
    raise exception 'Unsupported Calendar invitation recipient class.'
      using errcode = '22023';
  end if;

  new.recipient_type := canonical_recipient_type;
  return new;
end;
$$;

revoke all on function public.enforce_calendar_invite_recipient_type()
from public, anon, authenticated;

drop trigger if exists calendar_event_invites_recipient_type_contract
on public.calendar_event_invites;
create trigger calendar_event_invites_recipient_type_contract
before insert or update of recipient_type
on public.calendar_event_invites
for each row
execute function public.enforce_calendar_invite_recipient_type();
...
comment on function public.event_player_eligible_recipients(uuid, uuid, uuid[]) is
  'Resolves only current authorised Parent, guardian, and Adult Player recipients and emits the canonical Calendar invitation recipient contract.';
