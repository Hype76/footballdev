...
rename to authorize_match_day_push_v2_before_substitution_98;
...
begin
  if normalized_type <> 'substitution' then
    return public.authorize_match_day_push_v2_before_substitution_98(
      actor_user_id_value,
      match_day_id_value,
  ...
  return jsonb_build_object(
    'allowed', true,
    'targetParentLinkIds', to_jsonb(public.get_match_day_parent_notification_link_ids(match_row.id)),
...
revoke all on function public.authorize_match_day_push_v2_before_substitution_98(uuid, uuid, uuid, text, uuid) from public, anon, authenticated;
grant execute on function public.authorize_match_day_push_v2_before_substitution_98(uuid, uuid, uuid, text, uuid) to service_role;

comment on function public.authorize_match_day_push_v2(uuid, uuid, uuid, text, uuid) is
  'Server-authoritative Match Day push permission for saved active cards and substitutions.';
