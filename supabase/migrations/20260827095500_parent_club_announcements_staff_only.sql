-- Club Announcements are deliberate staff publications, not general outbound email history.
-- This migration changes only read and read-receipt functions. It does not mutate stored rows.
...
as $$
  ...
  join public.users author
    on author.id = log.user_id
    and author.club_id = log.club_id
    and author.status = 'active'
    and author.role_rank >= 20
  ...
  where public.current_user_can_access_parent_link(link.id, log.player_id)
    ...
    and lower(btrim(coalesce(log.metadata ->> 'source', ''))) = 'club_announcement'
    and lower(btrim(coalesce(log.metadata ->> 'authorType', ''))) = 'club_staff'
    and nullif(btrim(coalesce(log.metadata ->> 'body', '')), '') is not null
    and case
...
comment on function public.get_parent_portal_email_messages(uuid) is
  'Returns only explicit Club Announcements authored by active staff and addressed to the authenticated active Parent link.';

comment on function public.mark_parent_portal_message_read(uuid, uuid) is
  'Marks an explicit staff Club Announcement read only for its authenticated active Parent recipient.';
