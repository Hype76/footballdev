...
add column if not exists hide_votes boolean not null default false,
...
alter table public.polls
add constraint polls_poll_type_check check (poll_type in ('text', 'time', 'awards'));

drop index if exists public.poll_votes_poll_email_unique;

create unique index if not exists poll_votes_poll_email_option_unique
on public.poll_votes (poll_id, voter_email, option_id);

drop policy if exists poll_votes_delete_own_scoped on public.poll_votes;
create policy poll_votes_delete_own_scoped
